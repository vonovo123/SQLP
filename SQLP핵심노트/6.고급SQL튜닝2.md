...
7. RANGE + HASH 파티셔닝하고 Range 파티셔닝 키로 조회시 읽게 되는 파티션 세그먼트 개수
...
=> 동적 PRUNING 하더라도 실행할 때마다 딕셔너리를 참조하면서 읽어야 할 파티션을 결정하는 것은 아니므로 정적 PRUNING에 비해 성능 차이는 거의 없다.
...
PARTITION BY RANGE(주문일자)
(
PARTITION M01 VALUES LSEE THAN('20210201'),
PARTITION M02 VALUES LSEE THAN('20210301'),
PARTITION M03 VALUES LSEE THAN('20210401'),
PARTITION M04 VALUES LSEE THAN('20210501'),
PARTITION M05 VALUES LSEE THAN('20210601'),
PARTITION M06 VALUES LSEE THAN('20210701'),
PARTITION PMS VALUES LSEE THAN(MAXVALUE)
)
SELECT \* FROM 주문
WHERE
주문일자 lIKE '202103%';
...
문제 SQL을 위해 LOCAL PREFIXED 파티션 인덱스를 생성하려면, 인덱스 선두 컬럼이 '거래일자'이어야 한다. 이 경우 RANGE SCAN이 불가능하다.
...
조건절 컬럼인 계좌번호로 파티셔닝하는 GLOBAL PREFIXED 파티션 인덱스는 계좌번호가 선두컬럼이므로 RANGE SCAN이 가능하다. 루트에서 리프까지 DEPT가 비파티션 인덱스보다 낮아지도록 여러 개로 파티셔닝하면 인덱스 수직 탐색과정에 블록 i/o를 한 번 정도 줄일 수있다.

반면 파티션 크기를 균일하게 유지하기 위해 주기적으로 파티션을 추가해 주는 관리적 부담이 생긴다.
...
:CHG_DT 변수에는 어떤 값이든 입력할 수 있지만, 주로 당일 또는 전월 말일을 입력한다.
...
실시간에 가까운 정보 서비스 요구가 늘면서 'ON-DEMAND' 배치가 높고, 트랜잭션을 일정량 모았다가 지연처리하는 배치 프로그램의 활용도 늘어나는 추세다.
...
=> 병렬 처리에 사용한 병렬 프로세스는 모든 처리를 종료 (SELECT 문장의 커서를 닫거나 결과집합을 모두 FETCH) 한 후 일괄 해제하고 자원을 OS에 반환한다.
...
=> 데이터를 정렬 또는 그룹핑 하거나 조인을 위해 동적을 파티셔닝할때 사용하며, 첫 번째 병렬 서버 집합이 읽거나 가공한 데이터를 두 번째 병렬 서버 집합에 전송하는 과정에서 병렬 프로세스간 통신이 발생하므로 P->P는 INTER-OPERATION PARALLELISM에 속한다.
...
d.32
=> INDEX RANGE SCAN을 병렬로 처리할 때는 파티션 GRANUEL이므로 파티션 개수보다 많은 병렬도를 지정하면 서버 리소스를 낭비하게 된다.
...
38. 지정한 병렬도의 2배수로 프로세스를 할당하는 경우가 아닌 것
...
b. 파티션 인덱스가 아니면 INDEX RANGE SCAN, INDEX FULL SCAN은 병렬처리가 불가능하다.
...
SELECT DEPTNO, (CASE WHEN GROUPING(EMPNO) == 1 THEN 'RP' ELSE TO_CHAR(EMPNO)), SUM(SAL) 'SUMSAL', ROUND(AVG(SAL)) 'AVGSAL'
FROM EMP
GROUP BY DEPTNO, ROLLUP(EMPNO)
ORDER BY 1, 2;
...
SELECT A.고객번호, A.고객명,
(CASE WHEN B.연락처구분코드 = 'HOM' THEN B.연락처번호 END) 집
,(CASE WHEN B.연락처구분코드 = 'OFC' THEN B.연락처번호 END) 사무실
(CASE WHEN B.연락처구분코드 = 'MBL' THEN B.연락처번호 END) 휴대폰
FROM 고객 A, 고객연락처 B
WHERE A.고객번호 = B.고객번호
AND A.고객구분코드 = 'VIP'
GROUP BY 고객번호
...
SELECT
A.고객번호, MIN(A.고객명), LISTAGG('('||B.연락처구분코드||')'|| B.연락처번호) within group (order by 연락처구분코드) 연락처
FROM 고객 a, 고객연락처 b
WHERE a.고객구분코드 = 'VIP'
AND  B.고객번호 = A.고객번호
GROUP BY a.고객번호
...
하지만, 트랜잭션 수준 일관성은 여전히 보장되지 않는다. 즉, INSERT문을 시작한 이후에
...
SELECT 고객ID, SUM(입금액) , SUM(출금액) 출금액
FROM (
  SELECT 고객ID, 입금액, TO_NUMBER(NULL) 출금액
  FROM 입금
  UNION ALL
  SELECT 고객ID, 출금액, TO_NUMBER(NULL) 출금액
  FROM 출금
)
GROUP BY 고객ID
```
...
SELECT 지점코드, 판매월, 매출금액, SUM(매출금액) OVER(PARTITION BY 지점코드 ORDER BY 판매월 RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) 누적매출
FROM 월별지점매출
...
SELECT T1.지점코드, T1.판매월, MIN(T1.매출금액) 매출금액, SUM(T2.매출금액)
FROM 월별지점매출 T1, 월별지점매출 T2
WHERE T2.지점코드 = T1.지점코드
AND T2.판매월 <= T1.판매월
GROUP BY T1.지점코드, T1.판매월
...
FROM 장비 P
, (
  SELECT 장비번호, 변경일자, 변경순번, 상태코트,ROW_NUNBER() OVER (PARTITON BY 장비번호 ORDER BY 변경일자 DESC) RNUM
  FROM 상태변경이력
) H
WHERE H.장비번호 = P.장비번호
AND H.ROWNUM = 1;
