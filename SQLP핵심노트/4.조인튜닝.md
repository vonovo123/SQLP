...
조인되지 않을 불필요한 상품을 PGA에 적재하는 비효율도 없다.
...
Orderby 가 없는 인라인 뷰는 옵티마이저에 의해 MERGING 될 수 있으므로 정확한 실행계획을 위해 NO_MERGE힌트가 필요하다.
...
상품 기준으로 주문상품과 조인하면 첫 번째 FETCH CALL을 위한 ARRAY를 채우기까지
많은 데이터를 읽어야한다.

할인유형코드 = 'K890' 조건으로 판매되는 상품은 5000개이므로 50만개의 상품중 나머지 49만 5천개의 상품은 조인에 실패한다.
