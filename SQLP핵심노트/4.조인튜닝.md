...
FROM 절에 테이블을 나열한 순으로 조인하고자 할 때 force order 힌트를 사용한다.
NL 조인을 유도할때 'inner loop join table on (join condition)' 구문을 사용하며,
해시 조인으로 유도할 때 'inner hash join table on (join condition)' 구문을 사용한다.
...
인덱스 정보가 제공되지않지만 데이터 스캔건수와 테이블 엑세스를 통해 얻은 로우 건수를 통해 추론해볼 수 있다.
...
-- 368    INDEX RANGE SCAN 거래_X01 (CR=25257 PR = 3421)
```
...
하지만 NL조인을 우선 검토한 후에 성능 개선 방안을 찾기 어려울 때 해시 조인을 검토하는것이 올바른 순서다.
...
11. 주어진 SQL을 위한 PRA_HST_STC 테이블의 최적 인덱스
...
Outer 쪽 테이블 액세스가 NESTED LOOPS 위쪽에 위치하는 실행계획은 나타나지 않는다.
...
SELECT ...
,(
  SELECT CASE WHEN A.일할계산여부 = 'Y' THEN NVL(A.총청구건수, 0) - NVL(A.청구횟수 , 0)
  ELSE B.할부개월수 - NLV(A.청구횟수, 0) END
  FROM 서비스별할부 A, 할부계획 B
  WHERE A.서비스계약번호 = MV.서비스계약번호
  AND A.할부상태코드 = 'XR'
...
\- 해시 맵 탐색(PROBE PHASE) : 큰 테이블을 스캔하면서 해시 맵을 탐색한다. 조인에 성공한 데이터는 클라이언트에 전송한다.
...
해시 조인은 해시 알고리즘 특성상, 조인 조건 중 하나 이상이 = 조건일 때만 사용할 수 있다.
...
3개월 판매실적은 300만건 정도인데 상품코드 In 조건으로 필터링한 결과는 9만건 이므로 인덱스 스캔 비효율이 크다
...
select /*+leading(b) use_nl(a)* /A.대리점명, B.판매금액
FROM 대리점 A,
(
  select /*+no_merge*/
  대리점코드, sum(판매금액) 판매금액
...
b. 수행 빈도가 매우 높아 시스템 전체 성능에 미치는 영향도가 큰 쿼리에 사용한다.
...
주문과 주문상품을 조인한 집합으로 상품과 조인할 때는 상품이 PROBE INPUT이다.

probe input을 명시적으로 선택할 때는 no_swap_join_inputs 힌트를 사용한다.
...
SELECT O.주문번호, O.고객번호, (
  ...
  O.주문금액 AS 총주문금액
( SELECT MAX(주문금액) FROM 주문상품 WHERE 주문번호 = O.주문번호 ) AS 최대주문금액
FROM 주문 O
WHERE O.주문일자 BETWEEN :ORD_TD1 AND :ORD_DT2
...
b. 조인 순서를 변경한다.
=> 지점 테이블을 먼저 읽은 후 판매내역과 NL 조인한다, 조인 엑세스를 30번으로 줄일 수 있다. 판매내역\_PK 인덱스가 '판매지점코드 + 판매월'로 구성돼 있어 성능 개선이 가능하다.
...
- 고객당 하루 주문 건수가 많다.
- 주문일시 조건을 만족하는 데이터가 많다.
- 등록지점코드 조건을 만족하는 데이터가 많다.
...
튜닝 후 SQL의 성능 개선 효과를 극대화 하려면 고객 당 하루 주문 건수가 많아야하고, 주문일시 조건을 만족하는 주문 데이터가 많아야 하며, 등록지점코드 조건을 만족하는 고객 데이터도 많아야 한다.
...
-- 주문상품의 보관기간 = 10년
-- 주문상품 총 건수 = 1억2천만건
-- 할인유형코드 조건을 만족하는 데이터 비중 20%(20만건)
-- 등록된 상품 2만 개
-- 2만개 상품을 고르게 주문
...
주문상품은 비파티션 테이블이므로 1억 2천만건 중 20만 건을 찾으려면 인덱스가 필요하고 인덱스를 '할인유형코드 + 주문일시' 순으로 구성하면 효과적이다.

```sql
-- 주문상품_X1 : 할인유형코드 + 주문일시
```

상품은 대상레코드가 2만개이고 쿼리에 사용된 컬럼은 3개 뿐이다.

상품 데이터를 PGA에 충분히 담을 수 있고 2만 개 상품을 고르게 주문하므로 조인되지 않을 불필요한 상품을 PGA에 적재하는 비효율도 없다.

따라서 NL 조인을 통해 20만개의 주문상품에 맞는 상품레코드를 버퍼캐시에서 읽어오는 것 보다 2만개의 상품을 PGA에 담고 해시 조인 과정에서 탐색하는 것이 훨씩 효과적이다.

```sql
-- index 재구성 주문상품_x1: 할인유형코드 + 주문일시
SELECT /*+LEADING(P) USE_HASH(O) FULL(P) INDEX(O 주문상품\_x1) */
...
GROUP BY 후 조인하는 방법도 고려할 수 있다.

PGA에 충분히 담을 수 있는 집합이므로 GROUP BY 수행 부담을 무시할만하다.

2만개의 상품을 고르개 주문하므로 GROUP BY의 결과 집합은 2만여 건이다.

상품코드당 주문상품은 평균 10건이므로 조인횟수를 10분의 1로 줄일 수 있다.

GROUP BY 추가 수행에 따른 부담을 상쇄하고도 남는다.

조인 횟수가 줄고 같은 상품코드로 여러번 조인하지도 않으므로 NL 조인의 부담이 줄지만, 여기서도 해시 조인이 더 빠르고 효과적이다.

상품 데이터를 PGA에 충분히 담을 수 있을 뿐아니라 2만개 상품을 고르게 주문하므로 불필요한 상품을 PGA에 적제하는 비효율도 없다.

해시 조인은 출력 순서를 보장하지 않으므로 ORDER BY를 맨 마지막에 기술해야 한다.
ORDER BY가 없는 인라인 뷰는 옵티마이저에 의해 merge 될 수 있으므로 정확히 위 실행계획이 나오게 하려면 No_MERGE 힌트가 필요하다.
...
-- 할인유형코드 조건을 만족하는 주문상품 데이터 비중 20%
-- 등록된 상품 = 2만개
...
주문 상품은 월 단위 파티션 테이블이므로 인덱스로 20만 건을 스캔 후 랜덤 액세스 하는 것 보다 애초에 full scan 하는 것이 유리하다.
...
조인 횟수가 줄고 같은 상품코드로 여러번 조인하지 않으므로 NL 조인의 부담이 줄지만, 해시 조인이 더 빠르고 효과적이다.

상품 데이터를 PGA에 충분히 담을 수 있을 뿐아니라 '2만개의 상품을 고르개 주문'하므로 불필요한 상품을 PGA에 적재하는 비효율도 없기 때문이다.
...
한 달 주문상품 100만건 중 할인유형코드 = 'K890' 조건을 만족하는 데이터는 20만 건이다.

주문 상품은 월 단위 파티션 테이블로 인덱스로 20만 건을 랜덤 엑세스하는 것 보다 FULl SCAN이 유리하다.
...
NL 조인는 출력 순서를 보장하지만 전체범위 처리이므로 굳이 ORDER BY를 인라인 뷰에서 처리할 이유가 없다.

Order by 가 없는 인라인 뷰는 옵티마이저에 의해 MERGING 될 수 있으므로 정확한 실행계획을 위해 NO_MERGE힌트가 필요하다.
...
주문상품은 월 단위 파티션 테이블 이므로 인덱스로 20만 건을 랜덤 액세스 하는 것 보다 full scan이 유리하다.
...
부분범위 처리를 활용하려면 소트연산을 생략할 수 있어야한다.

즉, 인덱스를 활용해 소트연산을 생략해야한다.

소트 연산을 생략하고 상품테이블의 등록일시 컬럼 기준 역순으로 정렬된 결과집합을 빠르게 출력하려면,

등록일시가 선두인 상품 인덱스를 역순으로 스캔하면서 주문상품 GROUP BY 집합과 NL조인하면 된다.

조인 시 GROUP BY 집합 전체를 읽지 않도록 하기 위 JOIN PREDICATE PUSHDOWN 기능이 작동해야 한다.

따라서 의도대로 정확히 실행되게 하려면 NO_MERGE , PUSH_PRED 힌트를 사용해야 한다.
...
--  NESTED LOOPS
--    TABLE ACCESS BY INDEX ROWID OF 상품
--       INDEX FULL SCAN DESENDING OF 상품_X1
...
52. 결과집합 중 상위 100개만 출력하고 멈추는 환경에서 sql 튜닝
...
--      HASH GROUP BY
--        NESTED LOOPS
--          PARTION RANGE ITERATOR
--            TABLE ACCESS BY LOCAL INDEX ROWID 주문상품
--              INDEX RANGE SCAN of 주문상품_x1
--          INDEX UNIQUE SCAN OF 상품_PK
--        TABLE ACCESS BY INDEX ROWID OF 상품
...
위 쿼리에 부분범위 처리를 활용하려면 상품의 등록일시 인덱스를 역순으로 스캔하면서 주문상품 GROUP BY 집합을 JOIN PREDICATE PUSHDOWN 방식으로 NL 조인해야한다.
...
상품 기준으로 주문상품과 조인하면 첫 번째 FETCH CALL을 위한 ARRAY를 채우기까지 많은 데이터를 읽어야한다.

할인유형코드 = 'K890' 조건으로 판매되는 상품은 5000개 이므로 50만개의 상품중 나머지 49만 5천개의 상품은 조인에 실패한다.

상위 100개 상품을 출력하려면 대략 10,000개 상품을 스캔하면서 주문상품과 조인하고 조건절을 필터링해야하므로 빠른 응답속도를 얻기 힘들다.

반면, 정렬 기준이 상품의 등록일시인 상황에서 아래와 같이 주문상품을 먼저 읽어 GROUP BY 하면 조인까지 모두 마쳐야 출력을 시작할 수 있다.

즉, 부분범위 처리의 이점을 활용하지 못한다.
...
50만개 상품 중 할인유형코드 = 'K890' 조건으로 판매되는 상품은 5,000개로 group by 결과 집합도 5,000개이다.

group by 후 상품과 NL 조인하면 50만개 중 5000개 상품만 인덱스로 읽어 조인하면 되지만, NL 조인의 특성상 느리다.

반면, group by 후 hash 조인하면, 50만개 상품을 모두 해시 맵에 올린 후 5천번 탐색한다.

PGA에서 탐색하므로 조인 과정의 성능은 HASH 조인이 우세하지만, 상품 테이블 컬럼이 많으면 해시 맵을 생성하기 위해 FULL SCAN 하는 과정에서 블록 I/O가 많이 발생한다. PGA 공간이 가득 차면, 그로 인한 성능 저하도 발생한다.

상품 인덱스를 [등록일시 + 상품코드] 또는 [상품코드 + 등록일시] 순으로 구성하고 인라인 뷰 내에서 인덱스만 읽어 해시 조인하면 정렬 기준인 등록일시를 빠르게 얻을 수 있다.

상품코드로 group by 하고 등록일시로 order by 까지 끝낸 집합을 기준으로 nl 조인하면 부분범위처리 할 수 있어 효과적이다.

```sql

-- 답안_1
-- 상품_X1 : 등록일시 + 상품코드 / 상품코드 + 등록일시

SELECT /*+LEADING(O) USE_NL(P) NO_NLJ_BATCHING(P)*/
P.상품코드, P.상품명, B.등록일시, P.상품가격, P.공급자ID, O.총주문수량, O.총주문금액
FROM (
  select /*+FULL(A) INDEX_FFS(B) LEADING(B) USE_HASH(A)*/
  A.상품코드, min(B.등록일시) 등록일시ss
...
--              PARTITON RANGE (ITERATOR)
--                TABLE ACCESS FULL OF 주문상품
--     INDEX UNIQUE SCAN OF 상품_PK
```

order by 가 있는 인라인 뷰는 옵티마이저에 의해 merging 되지 않으므로 no_merge 힌트는 불필요하다.

nl 조인 과정에서 i/o가 발생하면 출력 순서가 흐트러질 수 잇으므로 NO_NLJ_BATCHING 힌트를 추가해야 한다.

모범답안 2 처럼 인라인 뷰에서 상품\_x1 인덱스와 해시 조인할 때 rowid를 읽어 상품 테이블과 다시 조인할 때 사용하는 방법도 있다.
...
작업자ID가 'Z123456'인 작업자가 최근 한 달간 방문해서 처리한 작업 지시를 실제방문일자 역순으로 최근 10건만 출력하는 쿼리 작성
...
특정 작업자가 최근 한달간 방문해서 처리한 작업지시 중 실제방문일자 역순으로 최근 10건만 출력하는 sql이다.

최적 인덱스 구성은 '작업자id + 실제방문일자' 이다

고객번호와 주소정보는 개통접수 또는 장애 접수 테이블에서 읽어야 하는데,
제시한 발생규칙은 작업자가 나머지 두 테이블과 베타적 관계임을 설명하고 있다.
따라서 두 테이블을 outer 조인해서 선택적으로 값을 읽어야 한다.

접수구분은 작업지시의 개통접수번호와 장애접수번호 중 어느 쪽에 값이 입력됐는지 여부에 따라 구분해도 되고 개통접수와 장애접수 중 테이블과 조인에 성공했는지 여부에 따라 구분해도 된다.

부분범위 처리가 불가능하도록 전체 범위를 읽어야하는 경우,
작업지시 테이블을 조건에 맞게 먼저 10건 읽은 후 인라인 뷰 바깥에서 개통접수와 장애접수 테이블과 조인하는 것이 좋다.

54. 베타적 테이블 조인

아래의 데이터 모델과 인덱스 구성하에서 방문예정일자가 오늘인 작업지시 데이터를 모두 출력하는 쿼리 작성
...
-- 데이터 발생 규칙
-- 작업지시에 데이터를 입력할 때 작업구분코드 = A 이면 접수번호에 개통접수번호를 입력하고, 작업구분코드가 B이면 장애접수번호를 출력
...
작업지시\_x1 인덱스가 작업구분코드 + 방문예정일자 순으로 구성돼 있으므로 개통과 장애건을 각각 조회해서 union all로 결합했을때 발생하는 비효율은 없다
...
AND C.개통접수번호(+) = DECODE(A.작업구분코드, 'B', A.접수번호)
```

작업지시는 나머지 두 테이블과 베타적 관계이다.

작업구분코드를 기준으로 Union all하면 작업지시\_X1 인덱스의 방문예정일자 조건으로 두 번 스캔해야한다.

테이블 레코드도 각각 두 번 액세스한 후에 작업구분코드로 필터링해야 한다.
...
예를 들어 접수번호로 정렬한 결과집합에서 앞쪽 부분만 읽고 멈출 수 있는 상황이라면,
인덱스를 방문예정일자 + 접수번호 순으로 구성해주면된다.

UNION ALL로 작성한 경우, 인덱스를 이용한 소트 생략이 불가능하므로 부분범위처리도 불가능하다.

단, 결과집합을 정렬하지 않는 부분범위 처리는 가능하다.

56. 베타적 테이블 조인

접수일자가 오늘인 개통접수 및 장애접수 데이터 모두 출력하는 SQL과 최적 인덱스 설계
...
-- 데이터 발생 규칙
-- 작업지시에 데이터를 입력할 때 작업구분코드 = A 이면 접수번호에 개통접수번호를 입력하고, 작업구분코드가 B이면 장애접수번호를 입력
-- 작업지시_X1 : 작업구분코드 + 접수번호
-- 개통접수_X1 : 개통접수일시
-- 장애접수_X1 : 장애접수일시

SELECT /*+ordered use_nl(B)*/
B.작업일련번호, B.작업자ID, '개통' AS 작업구분
, A.고객번호
, A.주소
FROM 개통접수 A, 작업지시 B
WHERE A.개통접수번호 = B.접수번호
AND A.개통접수일시 >= trunc(sysdate)
...
, A.주소
FROM 장애접수 A, 작업지시 B
WHERE A.장애접수번호 = B.접수번호
AND A.장애접수일시 >= trunc(sysdate)
AND A.장애접수일시 < trunc(sysdate + 1)
AND B.작업구분코드 = 'B'
```

조회조건이 베타적 테이블에 있고 NL 조인으로 처리해야 유리한 소량의 결과집합이라면 UNION ALL 방식이 유리하다.

개통접수 및 장애접수 테이블이 드라이빙이 되도록 조인순서를 유도하고 그에 따른 최적 인덱스를 구성한다.

57. 점이력 쿼리작성

장비쿠분코드가 'A001' 인 장비의 최종상태코드, 변경일자, 변경순번을 출력
-- 장비구분코드가 'A001' 인 장비는 10건
-- 결과집합을 장비번호 순으로 정렬
...
장비구분코드가 'A001'인 장비의 최종상태코드와 직전일(상태변경이력의 변경일자가 장비의 최종상태변경일자보다 작은) 마지막 상태코드, 변경일자, 변경순번을 출력
...
, 변경순번 as 최종변경순번
  from (
    select 장비번호, 상태코드, 변경일자, 변경순번, ROW_NUMBER()
      OVER(PARTITION BY 장비번호 ORDER BY 변경일자 DESC, 변경순번 DESC) as RM
    from
    (
      select B.장비번호, B.상태코드, B.변경일자, B.변경순번
      from 장비 A, 상태변경이력 B
      where A.장비구분코드 = 'A001'
      AND A.장비번호 = B.장비번호
      and A.최종상태변경일자 > B.변경일자
    )
  )
  where rm = 1
  Order by 장비번호 desc
...
장비구분코드가 'A001' 인 장비의 최종 상태코드와 변경일자를 출력
...
SELECT A.장비번호, A.장비명, B.상태코드 AS 최종상태코드
, TO_CHAR(B.유효시작일자, 'YYYYMMDD') AS 최종상태변경일자
FROM 장비 A, 상태변경이력 B
WHERE A.장비구분코드 = 'A001'
AND B.장비번호 = A.장비번호
AND TRUNC(B.유효종료일시) >= sysdate
AND TRUNC(B.유효시작일시) <= sysdate
order by A.장비번호
```

상태변경이력은 선분이력 모델이력이다. 즉, 변경이 발생한 시점뿐만 아니라 해당 이력이 유효했던 종료 시점을 같이 관리하는 방식이다.

이 모델의 장점은 무엇보다 쿼리가 단순하다는데 있다. 쿼리가 단순해지면 성능도 좋아진다.
...
선분이력에서 최종 데이터의 유효종료일시에는 9999년 12월 31일 23시 59분 59초를 입력하는 경우라면 아래와 같이 = 조건절을 사용할 수도 있다.

WHERE 유효종료일시 = TO_DATE('99991231 235959', 'YYYYMMDD HHH24MISS')

60. 선분이력 쿼리작성

장비구분코드가 'A001' 인 장비의 최종 상태코드와 변경일자 그리고 직전( = 상태변경이력의 유효시작일시가 장비의 최종상태변경일보다 작은) 상태변경이력의 상태코드와 변경일자를 출력
...
-- 장비구분코드 = 'A001' 인 장비는 10건
-- 결과집합을 장비번호 순으로 정렬
-- 장비 테이블의 최종상태변경일시는 상태코드를 가장 마지막으로 변경한 일시
-- 상태변경이력은 선분이력 테이블
-- 장비 테이블의 최종변경일시와 상태변경이력의 유효시작일시, 유효종료일시는 DATE형

SELECT A.장비번호, A.장비명, P. 최종상태코드
, TO_CHAR(A.최종상태변경일시, 'YYYYMMDD') AS 최종변경일자
, B.상태코드 AS 직전상태코드
, TO_CHAR(B.유효시작일시, 'YYYYMMDD') AS 직전변경일자
FROM 장비 A, 상태변경이력 B
WHERE A.장비구분코드 = 'A001'
AND 장비번호 = A.장비번호
AND TRUNC(B.유효종료일시) >= A.최종상태변경일시 - 1/(60*60*24)
AND TRUNC(B.유효시작일시) < A.최종상태변경일시
order by A.장비번호
```
