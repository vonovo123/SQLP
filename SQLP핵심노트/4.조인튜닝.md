...
따라서 NL 조인 후 20만개의 주문상품을 버퍼케시에서 읽어오는 것 보다 2만개의 상품을 PGA에 담고 해시 조인 과정에서 탐색하는 것이 훨씩 효과적이다.

```sql
-- index 재구성 주문상품_x1: 할인유형코드 + 주문일시
SELECT /*+LEADING(P) USE_HASH(O) INDEX(O 주문상품\_x1) FULL(P)*/
...
할인유형코드 = 'K890' 조건으로 판매되는 상품은 5000개이므로 상위 100개 상품을 출력하려면 대략 10,000개 상품을 스캔하면서 주문상품과 조인하고 조건절을 필터링해야하므로 빠른 응답속도를 얻기 힘들다.

반면, 정렬 기준이 상품의 등록일시인 상황에서 주문상품을 먼저 읽어 GROUP BY 하면 조인까지 모두 마쳐야 출력을 시작할 수 있다. 즉, 부분범위 처리의 이점을 활용하지 못한다.
...
group by 후 NL 조인하면 50만개 중 5000개 상품만 인덱스로 읽어 조인하면 되지만,
NL 조인의 특성상 느리다.

반면, group by 후 hash 조인하면, 50만개 상품을 모두 해시 맵에 올린 후 5천번 탐색한다.

PGA에서 탐색하므로 조인 과정의 성능은 HASH 조인이 어세하지만, 상품 테이블 컬럼이 많으면 해시 맵을 생성하기 위해 FULL SCAN 하는 과정에서 블록 I/O가 많이 발생한다. PGA 공간이 가득 차면, 그로 인한 성능 저하도 발생한다.
