...
따라서 NL 조인 후 20만개의 주문상품을 버퍼캐시에서 읽어오는 것 보다 2만개의 상품을 PGA에 담고 해시 조인 과정에서 탐색하는 것이 훨씩 효과적이다.
...
2만 개 상품 중 할인유형 코드 조건으로 판매되는 상품은 10여 개 이므로 GROUP BY 결과 집합도 10여 건이다.

상품당 주문상품은 평균 2000건 이므로 GROUP BY를 먼저 처리하면 조인 횟수가 많이 감소한다.

GROUP BY 결과 집합은 10건이고 상품은 2만 개 이므로 해시조인보다 NL조인이 효과적이다.
...
총 10만건의 상품 중 상위 100개 상품을 출력하려면 대략 10,000개 상품을 스캔하면서 주문상품과 조인하고 조건절을 필터링해야하므로 빠른 응답속도를 얻기 힘들다.
...
FROM (
  SELECT /*+INDEX_DESC(a 작업지시_X1) USE_NL(b) USE_NL(C)*/
  a.작업일련번호, a.실제방문일자
...
select /*+leading(A) USE_NL(B) index(a 장비_x1)*/
A.장비번호, A.장비명
, B.최종상태코드
, B.최종변경일자
, B.최종변경순번
...
WHERE A.장비구분코드 = 'A001'
AND 장비번호 = A.장비번호
AND TRUNC(유효종료일시) >= sysdate
AND TRUNC(유효시작일시) <= sysdate
order by A.장비번호
```
