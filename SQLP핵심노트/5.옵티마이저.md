...
1. 옵티마이저에 대한 설명으로 부적절한 것

a. 규칙기반 옵티마이저는 미리 정해놓은 우선순위 규칙에 따라 액세스 경로를 평가한다.
b. 비용기반 옵티마이저는 미리 수집해 둔 통계정보를 이용해 액세스 경로를 평가한다.
c. 규칙기반 옵티마이저는 통계정보를 일부 활용한다.
=> 규칙기반 옵티마이저는 통계정보를 전혀 활용하지 않고 단순한 규칙에만 의존한다.
d. 비용기반 옵티마이저도 내부적으로 규칙을 사용한다.
...
a. 캐싱 효과를 고려하지 않는다, 즉, 모든 블록을 디스크에서 읽는다고 가정한다.
=>
...
b. 조걸절에 인덱스가 있으면 무조건 인덱스를 사용한다.

규칙기반 옵티마이저에서 FULL TABLE SCAN은 순위가 15위로 가장 낮기 때문에 무조건 인덱스를 사용한다.
하지만 고객유형 조건을 만족하는 고객이 전체에서 90%를 차지한다면 무조건 인덱스를 사용하는건 좋은 선택이 아니다.

c. order by 절에 인덱스가 있으면 무조건 인덱스를 사용해 소트 연산을 생략한다.
...
d. 부등호 조건절과 between 조건절 모두에 인덱스가 있으면 between 조건절의 인덱스를 사용한다.

인덱스 컬럼에 대한 Between 조건이 부등호 조건보다 높기때문에 between 조건절에 인덱스를 사용한다. between 은 닫힌 범위검색 조건이고, 부등호는 열린 범위검색 조건이므로
between이 더 유리하다는 규칙은 타당하다. 하지만 조건에 해당하는 데이터 수에 따라 이는 틀린 가정이 될 수 있다.

3. 비용기반 옵티마이저에 대한 설명으로 부적절한 것

a. 최적화에 유리한 형태로 쿼리르 변환한다.

b. 통계정보를 활용해 각 실행계획의 예상비용을 산정한다.

c. 사용자가 지정한 힌트대로 실행했을때와 옵티마이저가 선택한 실행계획의 비용을 비교해 최종 하나를 선택한다.
=> 옵티마이저는 가장 비용이 적게드는 액세스 경로를 선택한다. 그런데 사용자가 특정 인덱스를 특정 스캔방식으로 사용하도록 힌트를 지정했다면 옵티마이저는 비용이 더 높음에도 그 경로를 선택한다.
...
쿼리 오퍼레이션 각 단계의 선택도, 카디널리티, 비용을 계산하고, 궁극적으로 실행계획 전체에 대한 총 비용을 계산한다.
...
4. 오라클 옵티마이저 힌트가 무시되는 경우가 아닌 것.

a. 문법적으로 맞지 않게 힌트를 기술한 경우
b. 옵티마이저에 의해 내부적으로 쿼리가 변환된 경우
c. 사용자가 지정한 힌트대로 실행하면 비용이 더 높다고 판단되는 경우
=> 
사용자가 정확한 힌트를 제공하면 옵티마이저는 성능에 상관없이 그대로 따른다.
d. 논리적으로 불가능한 액세스 경로를 유도한 경우
...
\- 논리적으로 불가능한 액세스 경로
\- 의미적으로 맞지 않게 힌트 기술
\- 옵티마이저에 의해 내부적으로 쿼리변환
\- 버그

5. self-learning optimizer 기능과 거리가 먼것

a. Bind Variable Peeking
=> 
'bind viariable peeking'은 sql이 첫 번째 수행될 때 함께 딸려 온 바인드 변수 값을 훔쳐보고,
그 값에 대한 컬럼 분포를 응용해 실행계획을 결정하는 기능이다.
...
'adaptive cursor sharing' 은 처음 실행 시 특정 실행계획으로 실행했다가 바인드 변수에 다른 값이 입력됐을 때 예상보다 많은 I/O가 발생하면 다른 실행계획을
...
6. 비용기반 옵티마이저가 실행계획을 수립할 때 사용하는 정보와 거리가 먼 것.

a. 버퍼 캐시 크기
=> 
오라클 11g에서 도입된 Adaptive Direct Path Read 기능이 Direct path read를 사용할지 여부를 결정할 때 고려하는 항목 중 하나가 버퍼캐시 크기다. 하지만, 이는 실행계획 수립 할 때가 아니라 런타임 시 고려사항이다.

b. 인덱스 높이
c. 중복을 제거한 컬럼 값의 수
d. CPU 속도 및 디스크 I/O 속도

---

옵티마이저가 사용하는 통계정보로는 크게 '오브젝트 통계'와 '시스템 통계'가 있다.
...
7. 최적화 목표에 대한 설명으로 부적절한 것.

a. 전체 처리속도 최적화는 쿼리 결과집합 전체를 읽는 것을 전제로 시스템 리소스를 가장 적게 사용하는 실행계획을 선택한다.

b. 최초 응답속도 최적화는 전체 결과집합 중 앞쪽 일부만 읽다가 멈추는 것을 전제로 응답 속도가 빠른 실행계획을 선택한다.

c. 전체 처리속도 최적화와 비교할때 최초 응답속도 최적화는 TABLE FULL SCAN 보다 인덱스를 더 많이 선택하고, 해시조인보다 NL 조인을 더 많이 선택하는 경향을 보인다.

d. 웹 브라우저를 통해 온라인으로 상품을 조회하고 주문을 처리하는 쇼핑몰이라면 최적화 목표를 최초응답속도로 설정해야한다.
=> oltp성 애플리케이션이더라도 아키텍처에 따라 최적화 목표는 다를 수 있다.
...
8. 옵티마이저 행동에 영향을 미치는 요소와 거리가 먼 것.

a. 인덱스, IOT, 클러스터링, 파티셔닝 등 오브젝트 구성
b. PK, FK, CHECK, NOTNULL 등 DBMS 제약설정
c. 옵티마이저 모드
d. 네트워크 속도
...
9. 옵티마이저가 최적 실행계획 생성의 한계를 보이는 이유와 거리가 먼 것

a.비효율적인 인덱스 구성
b.부정확한 통계 및 결합 선택도 산정의 어려움
c. 최적화 시간에 허용된 시간 제약
d. 라이브러리 캐시 공간 제약
...
오라클의 경우 튜닝 모드에서 오프라인 옵티마이저를 구동하면, 시간 제약 없이 다이나믹 샘플링을 포함한 다양한 정보와 기법을 활용하므로 훨씬 더 완벽한 실행계획을 생성해낸다.
...
선택도 =직급 조건 선택도 \* 연봉 조건 선택도 = 0.25 \* 0.1 = 0.025
...
13. 시스템 통계에서 수집하는 항목이 아닌 것

a. 평균적인 single block i/o 속도
b. 평균적인 single block i/o 개수
=> 평균적인 Single block i/o 개수는 당연히 1이므로 수집하지 않느다.
c. 평균적인 multiblock i/o 속도
d. 평균적인 multiblock i/o 개수
...
14. 선텍도, 카디널리티, 비용에 대한 부적절한 설명

a. 선택도 : 조건절에 의해 선택될 것으로 예상되는 레코드 비중
b. 카디널리티 : 조건절에 의해 선택될 것으로 예상되는 레코드 수
c. i/o 비용 모델의 비용 : 쿼리 수행 과정에 읽을 것으로 예상되는 블록 수
=> 
...
d. cpu 비용 모델의 비용 : 예상 소요시간을 single block i/o 횟수로 환산한 값 
...
CPU 비용 모듈에서는 예상 I/O 시간과 예상 CPU 사용시간을 구한 후 SINGLE BLOCK i/o 시간으로 나눈 값을 비용 값으로 사용한다.

즉, 비용을 single block i/o가 소요되는 시간과의 상대적인 시간 비용으로 표현한 것이다.

실행계획상 100으로 표시시됐다면 '시스템에서 100번 single block i/o하는 정도의 시간이 소요될 것으로 옵티마이저가 예상한다.' 는 뜻이다.


15. 오라클 12c버전에서 사용하는 히스토그램 유형이 아닌 것
...
16. 인덱스를 이용한 테이블 액세스 비용을 계산할 때 사용하는 통계항목이 아닌 것
...
SQL을 캐시에서 찾아 곧바로 실행단계로 넘어가는 것을 '소프트파싱'이라 하고, 찾는데 실패해 최적화 및 로우 소스 생성 단계까지
모두 거치는 것을 '하드파싱'이라고 한다.
...
18. 하드 파싱 과정에서 일어나는 작업이 아닌 것.

a. 테이블 통계, 인덱스 통계, 컬럼 통계를 수집해 딕셔너리에 저장한다.
...
b. 테이블 구성, 인덱스 구성, 컬럼 구성에 관한 정보를 조회한다.
c. 옵티마이저 관련 파라미터를 참조한다.
d. 조인순서, 조인 메서드, 테이블 액세스 방식, 인덱스 스캔 방식, 사용 인덱스 등을 결정한다.
---
...
여러 사용자가 공유하면서 재사용한다. 캐시 공간이 부족하면 버려졌다가 다음에 다시 실행할 때 똑같은 최적화 과정을 거쳐 캐싱한다.


20. 바인드 변수에 대한 설명으로 부적절한 것

a. 바인드 변수를 사용하면 한 sql에 다른 값을 입력하면서 반복 재사용할 수 있다.
b. 바인드 변수를 사용하면 한 sql에 대한 실행계획을 여러 프로세스가 사용할 수 있다.
c. 바인드 변수를 사용하면 상수 값을 사용할 때보다 쿼리 성능이 좋아진다.
...
d. 바인드 변수를 사용하면 최적화 과정에 컬럼 히스토그램을 사용하지 못한다.
...
21. 라이브러리 캐시 최적화 방안과 가정 거리가 먼 것.

a. 커서를 공유할 수 있는 형태로 SQL을 작성한다.
b. 세션 커서 캐싱 기능을 활용한다.
c. 애플리케이션 커서 캐싱 기능을 활용한다.
d. open_cursors 파라미터를 낮게 설정한다.

---

open_cursors 파라미터는 세션 당 open 할 수 있는 커서의 개수를 제한하는 파라미터이다.

22. 커서에 대한 설명으로 가장 부적절한 것.

a. 공유 커서 : 라이브러리 캐시에 공유된 shared sql area

b. 세션 커서 : pga에 할당된 private sql area를 말한다.

c. 애플리케이션 커서 : 세션 커서를 제어하는 클라이언트 측 핸들로서 라이브러리에서 sql을 찾는 작업을 생략하고 반복 수행할 수 있는 커서를 말한다.

d. 묵시적 커서 : 라이브러리에서 sql을 찾는 작업을 생략하고 반복 수행할 수 있는 커서
...
23. 트레이스 결과를 통해 커서 동작 추론

a. 하드파싱 하지 않았다.
=> misses in library cache during parse 를 통해 최초 1회 하드 파싱이 일어난 사실을 알 수 있다.

b. SQL에 바인드 변수를 사용했다.
=> 바인드 변수를 사용하면 커서를 공유할 수 있으므로 하드 파싱이 한번만 일어난다. 하지만 상수 조건으로 반복 실행해도 커서를 공유할 수 있으므로 트레이스 결과만으로 바인드 변수 사용 여부를 알 수 없다.

c. 세션 커서 캐싱 기능이 작동했다.
=> 세션 커서 캐싱의 작동 여부는 트레이스 결과로 알 수 없다. v$open_cursor 뷰에서 해당 sql의 cursor_type이 'session cursor cached' 인지를 통해 확인 가능하다.

d. 애플리케이션 커서 캐싱 기능이 작동했다.
=> excute call이 5000번 일어낫는데 parse call 이 1회 일어났다면 애플리케이션 커서를 캐싱한 상태에서 반복 실행했음을 알 수 있다.

24. 정적/동적 sql 구분

```sql
-- (가) 토드, 오렌지 같은 쿼리 툴에서 실행
select count(*) from 사원 where 성별 = '남';
-- (나) 토드, 오렌지 같은 쿼리툴에서 :p 변수에 '남'을 입력하고 아래 sql 실행
select count(*) from 사원 where 성별 = :p;
-- (다) Java에서 sql 실행
String SQLState = "select count(*) from 사원 where 성별 = ?";
PreparedStatement stmt = conn.prepareStatement(SQLSate);
stmt.setString(1,'남');
...
-- (마) Java에서 sql 실행
if (P_성별 =='남'){
  String SQLState = "select count(*) from 사원 where 성별 = '남'";
} else {
  String SQLState = "select count(*) from 사원 where 성별 = '여'";
}
...
25. cursor_sharing 파라미터의 개념 및 활용 기준

a. Force로 설정하면, sql에 사용한 상수 값이 바인드 변수로 자동 변환된다.
b. Force로 설정하면, 바인드 변수를 사용하지 않음으로 인한 sql 하드파싱 부하를 줄이는데 도움을 준다.
c. Exact로 설정하면, sql 문장이 100% 일치할 때만 캐싱된 커서를 공유 및 재사용한다.
d. Exact로 설졍하면, SQL 하드파싱부하를 유발하므로 Force로 설정하는것이좋다
...
예상치 못한 현상이 발생할 수도 있는데, 아래 sql 조건절을 위해 함수기반 인덱스를 생성했는데, 포맷 문자열('YYYYMMDD)'까지 바인드 변수로 반환되는 바람에 인덱스를 사용하지 못하는 문제가 발생할 수 있다.
...
26. 쿼리 변환에 대한 설명으로 부적절한 것.

a.옵티마이저 내장 엔진 중 Query Transformer 가 담당한다.

b.결과만 보장된다면 옵티마이저는 무조건 쿼리를 변환하고, 변환한 쿼리를 기준으로 가능한 후보군 실행계획 중 하나를 선택한다.
=> 쿼리 변환 시 항상 더 나은 성능을 보장하는 일부 경우를 제외하고 대부분 비용기반쿼리변환으로 작동한다. 비용기반 쿼리변환은 예상비용이 낮을 때만 변환된 쿼리를 사용한다. 따라서 변환 전 쿼리에 대한 후보군 실행계획과 변환 후 쿼리에 대한 후보군 실행계획을 비교해 비용이 가장 낮은 것을 선택한다.

c.옵티마이저가 다양한 쿼리 변환을 시도할 수록 SQL 최적화 시간은 늘어난다.

d.sql문에 쿼리 변환과 관련한 힌트를 지정하면, 옵티마이저는 그것을 우선해서 적용한다.

---

쿼리 변환은 옵티마이저가 SQL을 분석해 의미적으로 동일하면서도 더 나은 성능이 기대되는 형태로 재작성하는 것을 말한다.

결과만 보장된다면 무조건 쿼리 변환을 수행하는 것을 휴리스틱 쿼리변환이라 한다.
...
select
from customer c
where c.regdate >= trunc(add_months(sysdate, -1), 'mm')
and exists (
  select 'x'
  from 거래
  where 고객번호 = c.고객번호
  and 거래일자 >= trunc(sysdate, 'mm')
)
```

a. 고객과 거래 테이블을 조인하면 결과집합이 거래 단위로 변하므로 unnesting 할 수 없다.

=> 서브쿼리를 Unnsting 하면 고객을 먼저 드라이빙할 수도 있고, 거래를 먼저 드라이빙할 수도 있다.

단, 고객을 먼저 드라이빙하면 결과집합이 M집합인 거래 단위로 변하지 않도록 semi 조인 방식을 사용한다.

거래를 먼저 드라이빙하는 경우 sort unique 연산을 통해 고객 단위의 Unique한 거래집합을 생성한 후 고객 테이블과 조인한다.

b. 서브쿼리를 unnesting 하지 않으면, 메인 쿼리와 서브쿼리를 각각 최적화한다.
c. 서브쿼리를 unnesting 하지 않으면, 고객을 기준으로 거래 테이블과 nl 방식으로 조인한다.
d. 서브쿼리를 unnesting 하지 않으면, 서브쿼리에 대한 캐싱 기능이 작동한다.

---

서브쿼리는 하나의 sql 문장 내에서 괄호로 묶은 별도의 쿼리 블록을 말한다.
인라인 뷰, 중첩된 서브쿼리, 스칼라 서브쿼리가 서브쿼리에 속한다.
...
-- SELECT STATEMENT
--    FILTER
--      TABLE ACCESS FULL ON CUST
--      INDEX RANGE SCAN OF 거래_PK
```

=> 서브쿼리를 Unnesting 하지않고 고객을 기준으로 Filter 방식으로 조인했을때의 실행계획이다.
...
-- SELECT STATEMENT
--    NESTED LOOPS(SEMI)
--      TABLE ACCESS FULL ON 고객
--      TABLE ACCESS FULL ON 거래_PK
```

=> 서브쿼리를 Unnesting 한 후 고객을 기준으로 거래 테이블과 NL 세미조인할때의 실행계획
...
=> 서브쿼리를 Unnesting 한 후 고객 단위 집합으로 가공한 (sort unique) 거래 데이터를 기준으로 고객 테이블과 조인할때의 실행계획

29. 중첩된 서브쿼리가 필터방식으로 동작하는 것을 방지하는 힌트
...
서브쿼리를 Unnesting 하지 않으면 메인 쿼리를 기준으로 서브쿼리를 반복 실행하는 Filter 방식으로 처리할 수 밖에 없다.

필터 방식을 쉽게 이해하려면, filter 오퍼레이션을 nested loops 로 치환해서 해석하면 된다.

필터는 기본적으로 nl 조인과 같은 방식이라 대용량 데이터 처리에 매우 불리하지만
내부적으로 서브쿼리 캐싱 기능을 적용하므로 서브쿼리에 입력할 수 있는 값의 종류가 적을 때는 성능이 크게 나쁘지 않다.

캐싱 기능을 이용하기 위해 항상 이 방식으로 실행하도록 강제하려면 no_unnest 힌트를 사용하면 된다.

30. SEMI NL JOIN을 유도하는 힌트
...
위 실행계획은 서브쿼리를 unnesting 했을 때 나타나는 여러 실행계획 중 하나다.

서브쿼리를 unnesting 하지 않으면 nl조인처럼 메인쿼리를 기준으로 서브쿼리를 반복 실행하는 필터 방식으로 처리한다.

Unnesting 하면 nl 조인뿐 아니라 해시조인, 소트머지조인으로도 처리할 수 있다.

조인 순서도 자유롭게 결정할 수 있는데, 위 쿼리는 1쪽 집합인 고객 테이블을 먼저 드라이빙한다.
...
이 방식으로 유도하려면, 우선 서브쿼리가 Unnesting 되지 않도록 Unnest 힌트를 사용하고 nl 세미 조인으로 유도하는 nl_sj 힌트를 사용한다.

31. M쪽 집합을 outer 집합으로 지정하고 SORT UNIQUE 후 NL 조인하도록 유도하는 힌트
...
조인 순서도 자유롭게 결정할 수 있는데, 위 쿼리는 서브쿼리 집합이자 M쪽 집합인 거래를 먼저 드라이빙했다.

고객과 거래는 1:M 관계이므로 그대로 조인하면 결과집합이 아닌 거래 단위의 집합으로 변한다.

고객 단위의 결과집합을 추출해야하므로 sort unique 연산을 통해 고객 단위 Unique 한 집합을 생성한 후 고객 테이블과 조인했다.

이 방식으로 유도하려면 우선 서브커리가 Unnesting 되도록 unnest 힌트를 사용해야한다.

그리고 거래 테이블이 드라이빙이 되도록 leading 힌트를 추가하고 use_nl 힌트로 nl 조인을 유도한다.

qb_name 힌트는 쿼리 블록에 이름을 지정할 때 사용한다.

여기서 unnest 힌트의 대상 서브쿼리 블록을 명시하고, 서브쿼리 내에 위치한 거래 테이블을 leading 힌트에서 명확히 참조하기 위해 사용한다.
...
가입한 지 1개월이 지났는데 거래내역이 없는 고객 수 조회하는 쿼리이다.
100만 건의 고객 레코드 중 거의 대부분은 가입 후 1개월이 지났을 것이므로 고객 테이블은 Full scan으로 처리한다.

거래는 1000만건의 대량 테이블이므로 해시 방식으로 조인한다.

튜닝을 위해서는 서브쿼리의 Rownum 조건을 제거해야 한다.

unnesting 하지않은 중첩 서브쿼리는 항상 필터 방식으로 처리되므로 해시 조인을 위해서는 unnesting이 필요한데, 서브쿼리에 rownum을 사용하면 Unnesting이 불가능하다.

서브쿼리 Unnesting을 위해 unnest 힌트를 사용한다.

NOT EXISTS 서브쿼리이므로 해시 ANTI 조인으로 유도하기 위해 hash_aj 힌트를 사용한다.

마지막으로, 거래 테이블도 full scan 처리해야하는데, 다행히 인덱스만 읽고서도 처리 가능하다.

INDEX FAST FULL SCAN 을 할 수 있는 조건을 만족하므로 index_ffs 힌트를 추가한다.
...
--    HASH ANTI JOIN
--      TABLE ACCESS FULL OF '고객'
--      INDEX FAST FULL SCAN OF 거래_PK
```

33. 메인쿼리의 조인 레코드 수를 줄이기 위해 힌트를 통해 서브쿼리 필터링을 먼저 실행하는 방법
...
-- 1000           table acccess full 상품             (cr = 95)
-- 60000          table access by index rowid 주문    (cr = 38097)
-- 60000            index range scan 주문_X1          (cr = 2002)
-- 1              table access by index rowid 상품분류 (cr = 6)
-- 3                index unique scan 상품분류_pk       (cr = 3)
```
...
60,000 개의 조인 결과집합은 서브쿼리 filter 후 3000개로 줄었다.

총 읽은 블록수는 38,103이다. 대부분의 i/o가 상품과 주문 테이블의 조인에서 발생했다.

만약 상품과 상품분류 테이블 간의 서브쿼리 필터링을 먼저 처리하면 조인 단계로 넘어가는 로우 수를 크게 줄일 수 있다. 성능은 그만큼 향상된다.

주문 테이블과 조인하기 전에 서브쿼리 필터링을 먼저 처리하도록 push_subq 힌트를 사용한 한후 수집된 트레이스 결과다.
...
-- 3000       table access by index rowid 주문
-- 3000         index range scan 주문_X1
```

서브쿼리로 먼저 필터링한 결과가 150건이므로 주문 테이블과 조인 횟수도 150 번으로 줄었고, 주문 데이터도 3000개만 읽었다. 총 읽은 블록 수도 1903으로 줄었다.

push_subq힌트롤 통해 pushing한 서브쿼리의 필터링을 가능한 한 앞 단계에서 처리한다.

단, 이 기능은 Unnesting 되지 않은 서브쿼리에만 작동한다.

서브쿼리가 unnesting 되면 필터가 아닌 다양한 조인 방식으로 실행된다.

Unnesting 되는 순간 push_subq은 무용지물이다. 따라서 push_subq 힌트는 항상 no_unnestig 힌트와 같이 기술하는 것이 바람직하다.
...
a. 인라인 뷰를 merging 하지 않으면, 메인쿼리와 인라인뷰를 각각 최적화한다.

b. 인라인 뷰를 merging하지 않은 상태에서 인라인 뷰 집합을 기준으로 고객과 nl 조인하는 방식은 고객별 거래량이 많거나 가입일시 조건을 만족하는 데이터가 많을 때보다 고객별 거래량이 적고 가입일시 조건을 만족하는 데이터가 적을 수록 유리하다.

=> 인라인 뷰를 merging 하지 않은 상태에서 인라인뷰를 기준으로 고객과 nl 조인하는 방식은 인라인 뷰에서 읽어온 거래에 속하는 매인쿼리의 고객이 많을 때 유리하다.

가입일시 조건을 만족하는 수가 적으면 인라인 뷰 안에서 전체 고객을 대상으로 Groupby 하는 비효율이 상대적으로 커지기 때문이다.

c. 인라인 뷰를 merging하지 않은 상태에서 고객을 기준으로 인라인 뷰 집합과 nl 조인할 때, 최신 오라클 옵티마이저는 부분범위 처리 가능한 방식으로 처리하기도한다.

d. 인라인 뷰를 merging 하면 부분범위처리가 불가능하다.

---

최적화 단위가 쿼리 블록이므로 옵티마이저가 뷰 쿼리를 변환하지 않으면 뷰 쿼리 블록을 독립적으로 최적화 한다.

위 쿼리의 뷰를 독립적으로 최적화하려면 당월 거래 전체를 읽어 고객번호 수준으로 group by 한 후 고객테이블과 조인해야 한다.

고객 테이블에서 전월 이후 가입한 고객을 필터링하는 조건이 인라인 뷰 밖에 있다.

전월 이후 가입한 사람이 소수라면 인라인 뷰 안에서 당월에 거래한 모든 고객의 거래 데이터를 읽어 group by 하는 것은 비효율이 있다.

그럴 때 뷰를 merging 해서 고객 기준으로 거래와 조인한 후 group by 하면 전월 이후 가입한 고객이 당월에 일으킨 거래 데이터만 읽을 수 있어 효과적이다.

단, 이때 부분범위 처리는 불가능하다. group by 이후 데이터를 출력해야 하기 때문이다.

부분범위 처리를 이용해 빠른 응답속도를 내야한다면, view merging을 하지 않은 상태에서 조건절로 필터링된 고객을 기준으로 인라인 뷰 집합과 nl 조인하면 된다.

그러면 전월 이후 가입한 고객만을 대상으로 고객번호를 건건이 뷰 안으로 밀어넣으면서 각 고객별 당월 거래 데이터만 읽어서 group by 결과를 출력할 수 있다.

이 기능을 '조인 조건 Pushdown'이라고 한다.

오라클 11g 이후부터 작동하기 시작했다.

이 기능의 특장점은 nl 조인을 수행하는 도중에 멈출 수 있어 부분범위 처리가 가능하다는 점이다.

35. 인라인 뷰와 조인할때 나올 수 있는 실행계획
...
-- a. View merge 안하고 인라인뷰 기준으로 매인쿼리와 nl 조인
-- select statement
--  nested loops
--     view
--      hash group by
...
-- b. View merge 안하고 메인쿼리 기준으로 인라인뷰와 nl 조인
-- 조인조건 push down이 발생하지 않아 고객번호가 인라인뷰의 조건절로 쓰이지 않으므로
-- (view pushed predeciate 오퍼레이션 없음)
...
-- c. view mergeing 하고 고객과 거래테이블을 해시 조인 후 group by
-- select statement
--  HASH GROUP BY
--    hash join
--      table access full of 고객
...
-- d. view merging 하지않은 상태로 조인 조건 Pushdown 방식으로 조인.
...
따라서 뷰 안쪽에 no_merge 힌트를 사용하거나, 메인 쿼리에 no_merge(inline_view_alias) 힌트를 사용하면 된다.

뷰집합을 먼저 드라이빙 했으므로 leading(view_alias)를 추가하고, nl 조인이므로 use_nl 힌트를 추가하면 된다.
...
push pred를 위해서는 no_merge 힌트가 반드시 선제되야한다. 조인 조건 Pushdown 기능이 작동하려면 뷰가 merge 되지 않아야 하기 때문이다. 만약 view가 merge된다면 push_pred 힌트는 무시된다.

39. 부분범위처리가 가능한 인라인뷰 조인쿼리
...
from (
  ...
  (
    select
    lpad(avg(거래금액), 10)||lpad(min(거래금액), 10) ||lpad(max(거래금액),10)
    from 거래
    where 거래일시 >= trunc(sysdate, 'mm')
    and 고객번호 = c.고객번호
  ) 거래금액
...
40. 인라인 뷰를 사용했으나 View 오퍼레이션이 나타나지 않는 경우 실행된 쿼리변환
...
--      table access by index rowid 주문_x1
--        index range scan
```
...
조건절 Transitive (조건절 이행) 쿼리 변환은 a = b 이고 b = c 이면 a = c 이다는 추론을 통해 새로운 조건절을 내부적으로 생성해주는 쿼리 변환이다.

a > b 이고 b > c 이면 a > c이다 라는 추론이 가능하다.
예를 들어, a 테이블에 사용된 필터 조건이 조인 조건절을 타고 반대편 b 테이블에 대한 필터 조건으로 이행 될 수 있다.

즉, 한 테이블 내에서 두 컬럼간 관계정보를 이용해 조걸절이 이행된다.

위 쿼리에서는 c.고객번호가 = 1234 이고 o.고객번호 = c.고객번호이므로 o.고객번호 = 1234 라는 추론이 가능하다.
...
a.concatenate
b.use_concat
c.index(고객, 고객\_x2, 고객\_x3)
d.index(고객, 고객\_x2) index(고객, 고객\_x3)
...
OR 조건을 이 방식으로 실행하도록 유도하고자 할때 use_concat 힌트를 사용한다.

만약 이 기능을 방지하기 위해 NO_Expand를 사용한다면, 다음과 같은 실행계획이 나타난다.
...
44. or_expansion 을 유도하는 힌트와 유도하지 않는 힌트
...
select *
from emp e, dept d
where (e.deptno = d.deptno and e.job = 'CLEARK' and d.loc = 'DALLAS')
or
(e.deptno = d.deptno and e.job = 'CLEARK' and e.mgr = 7782)

```

공통 표현식 제거는 같은 조건식이 여러 곳에서 반복 사용될 경우, 해당 조건식이 각 로우당 한 번씩만 평가되도록 변환하는 것을 말한다.

옵티마이저가 두 조건절에 공통으로 사용된 조건절을 바깥을 빼내서 아래 정답과 같이 쿼리를 재작성할 수 있고, 그러면 문제에 제시한 실행계획이 나타난다.
...
둘 중 어느 쪽의 성능에 유리할지는 데이터 분포에 따라 다르다.
