...
26. 쿼리 변한에 대한 설명으로 부적절한 것.

a.옵티마이저 내장 엔진 중 Query Transformer 가 담당한다.ss
...
=> 서브쿼리를 Unnesting 한 후 고객을 기준으로 거래 테이블괴 Nl 세미조인할때의 실행계획
...
조건절 Transitive (조건절 이행) 쿼리 변환은 a = b 이고 b = c 이면 a = c 이다는 추론을 통해 새로운 조건절ㅇ르 내부적으로 생성해주는 쿼리 변환이다.
