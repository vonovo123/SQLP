...
1. B\*Tree 인덱스 구조에 대한 설명으로 가장 부적절한 것

a. 브랜치 블록의 각 로우는 하위 블록에 대한 주소값을 갖는다. => ROWID
b. 브랜치 블록 각 로우의 키 값은 하위 블록 첫 번째 로우의 키 값과 일치한다. =>
브랜치 블록 각 로우의 키 값은 하위 블록이 갖는 값의 '범위'를 의미한다. 따라서 하위 블록 첫 번째 로우의 키 값과 일치하지 않을 수 있다.
c. 리프 블록의 각 로우의 키 값은 테이블 로우의 키 값과 일치한다.
d. 리프 블록은 이중연결리스트 구조다

---

- 브랜치 블록의 각 로우는 하위 블록에 대한 '주소값'을 갖는다.
- 브랜치 블록 각 로우의 키 값은 하위 블록이 갖는 값의 '범위'를 의미한다.
- LMC는 다음 로우의 키 값보다 작은 키값들로 구성된 하위 블록과 연결된다.
...
2. 오라클 인덱스 ROWID를 구성하는 요소가 아닌 것은?
...
INDEX ROWID = DBA(데이터오브젝트번호 + 데이터파일번호 + 블록번호) + 블록내의 로우번호
...
1(루트블록)-2(브랜치블록)-6(리프블록)-7(리프블록)-8(리프블록))
...
select /*+index(C)*/ custnm, sex, regdt, pnum
select /*+index(C CUST_X1)*/ custnm, sex, regdt, pnum
select /*+index(C (REGDT))*/ custnm, sex, regdt, pnum
from cust C
...
WITH 절을 이용해 INDEX 힌트를 기술할 때는 일반적으로 2번 3번을 사용한다.
...
FORCESCAN을 지정하면 고객번호로 검색하지않고 FULL SCAN 하면서 고객번호를 필터링한다.
...
클러스터형 인덱스가 있는 상황에서 WITH 절에 INEX(0)을 지정하면 클러스형 인덱스를 SCAN하면서 고객번호로 필터링한다.
WITH 절에 INDEX(1)을 지정하면 고객번호로 클러스터형 인덱스를 검색한다.

클러스터형인덱스가 없는 상황이라면 WITH 절에 INEX(0)을 지정하면 테이블을 Full scan 하면서 조건절을 필터링하며, 1을 지정하면 구문오류가 발생한다.
...
6. SQL Server TABLE FULL SCAN 유도하는 방법으로 가장 부적절한 것
...
-- C
-- WITH(FORCESCAN) 으로 TABLE FULL SCAN을 유도한다.
SELECT 고객번호, 성별, 가입일자, 연락처
FROM 고객 WITH(FORCESCAN)
WHERE 고객명 = @CUST_NM
...
7. INDEX RANGE SCAN에 대한 설명으로 가장 부적절한 것
...
c. 인덱스 루트에서 리프 블록까지 수직 탐색한 후 리프 블록을 수평 탐색하는 방식이다.

d. 수평 탐색 범위는 인덱스 구성, 조건절 연산자에 따라 달라진다.

8. INDEX FULL SCAN에 대한 설명으로 부적절한 것
...
10. ORACLE INDEX FAST FULL SCAN에 대한 설명으로 부적절한 것
...
d. 테이블 파티션
=> 테이블 파티션은 사용자가 지정한 기준에 따라 데이터를 세그먼트 단위로 저장한다. 특정 조건을 만족하는 데이터를 인덱스를 이용한 랜덤 액세스가 아닌 TABLE FULL SCAN으로 빠르게 찾을 수 있다.
...
22. INDEX RANGE SCAN 가능한 쿼리

A. SELECT \* FROM COR WHERE CORNAME LIKE '%KOREA%'
=> 중간 값(양쪽 % LIKE 조건)으로는 인덱스 RANGE SCAN을 할 수 없다.
B. SELECT \* FROM COR WHERE CORNAME NOT LIKE 'KOREA%'
=> 부정형 비교는 INDEX RANGE SCAN을 할 수 없다.
C. SELECT \* FROM COR WHERE SUBSTR(CORNAME, 1,2) = 'KOREA'
=> 인덱스 컬럼을 변형하면 인덱스 RANGE SCAN을 할 수 없다.
D. SELECT \* FROM COR WHERE CORNAME = NVL(:VAL, 'KOREA')
...
```sql
WHERE CORNAME = :VAL OR :VAL IS NULL
```
...
따라서 단일 컬럼에 생성한 인덱스에 대한 IS NULL 조건으로는 INDEX RANGE SCAN이 불가하다. 반면, 결합 인덱스는 구정 컬럼중 하나라도 NULL 이 아니라면 IS NULL 조건에 대한 INDEX RANGE SCAN이 가능할 수 있다.
...
=> 숫자형과 문자형을 비교할때는 '숫자형을 기준으로 문자형이 자동 변환'된다.
따라서 인덱스 컬럼 자동 형변환이 발생하므로 정상적인 INDEX RANGE SCAN이 불가하다.

b. select \* from 계좌 where 계좌번호 like :acnt_no || '%'
=> 일반적인 자동형변환은 숫자형을 기준으로 문자형으로 형변환 하지만 연산자가 LIKE 라면 예외적으로 문자형을 기준으로 숫자형을 자동 변환한다. 숫자형인 계좌번호 컬럼이 문자형으로 자동형변환 된다. 인덱스 컬럼에 자동 형변환이 발생하므로 정상적인 INDEX RANGE SCAN이 불가하다.

c. select \* from 계좌 where 지정코드 IS NULL;
=> 지점코드를 선두로 갖는 인덱스 계좌\_X1을 사용할 수 없다. 인덱스를 구성하는 컬럼 모두가 NULL 허용컬럼이기 때문이다.
두 컬럼 값이 모두 NULL이면 인덱스에 저장하지 않으므로 지정코드 IS NULL로 검색하면 결과집합에 누락이 발생한다.
...
계좌\_X2인덱스를 INDEX FULL SCAN 할 수는 있다.
...
or (:opt = 'B' and phonenum = :mobile_no)
...
27. index range scan이 가능하도록 재작성

```sql
-- 1
select *
from Vender
where SUBSTR(vender,1,2) like '대한'
...
-- 답안 3
SELECT /*+NO_MERGE ORDERED USE_NL(B)*/
...
-- 답안 4
SELECT /*+USE_CONCAT*/ 주문번호, 주문일시, 고객ID, 총주문금액, 처리상태
FROM 주문
...
, SUM(DECODE(지수구부코드, '2', 지수종가 , 0))
, SUM(DECODE(지수구부코드, '2', 누적거래량 , 0))
FROM 일별지수업종별거래 A
...
인덱스 컬럼 주문번호를 가공(+1) 했으므로 FIRST ROW(MIN/MAX) 방식으로 작동하지 않았다. 참고로 아직 주문데이터가 생성되지 않은 일자를 입력하면 MAX 함수가 NULL을 반환하므로 MVL 처리는 반드시 필요하다.
...
버퍼캐시는 시스템 공유 메모리(SGA)에 위치하므로 직렬화를 위한 lock 메커니즘이 필요하다.

따라서 버퍼캐시에서 블록을 읽을 때마다 latch와 buffer lock을 획득해야 한다.

이로인해 경합이 발생하므로 인덱스 rowid를 이용한 테이블 액세스는 고비용이다.

33. 인덱스 클러스터링 팩터에 대한 설명으로 부적절한 것

a. 인덱스 컬럼을 기준으로 같은 값을 같는 데이터가 테이블에 서로 모여있는 정도를 의미
...
a. 인덱스 클러스터링 팩터가 좋을수록 손익분기점은 높아진다.
...
=> 테이블 전체 스캔 시 디스크 i/o 횟수가 줄어 테이블 전체 스캔 비용이 낮아진다. 따라서 인덱스 손익분기점은 낮아진다. 인덱스를 사용하는 방식에 비해 테이블 풀스캔의 효율성이 커진다.
...
데이터양이 늘면 버퍼캐시히트율이 점점 낮아진다. 버퍼캐시의 크기는 일정한데 적재할 데이터가 많을 수록 캐싱되어있을 확률이 낮아지기 때문이다. multoblock i/o 효과까지 고려하면, 데이터가 늘 때의 full scan 비용 상승보다 위 두 요소에 의한 랜덤 액세스 비용 상승이 더 크다. 따라서 데이터양이 늘수록 인덱스 손익분기점은 내려갈 확률이 높다.

d. 일정량 이상의 데이터를 읽을 때 인덱스 효용이 낮은 이유는 인덱스를 이용한 테이블 액세스가 랜덤 방식인 데다 Single Block I/O로 읽기때문이다.

35. OLTP와 BATCH 프로그램의 성능 특성
...
36. 인덱스 구성변경에 따른 영향도를 최소화하면서 쿼리 성능을 높이는 튜닝방안

```sql
-- N2 : 서비스번호 + 사용여부
SELECT 렌탈관리번호, 고객명, 서비스관리번호, 서비스번호, 예약접수일시, 방문국가코드,
로밍승인번호, 자동로밍여부
FROM 로밍렌탈
...
로밍렌탈\_N2 인덱스 구성을 서비스번호 + 사용여부 + 자동로밍여부로 변경한다.
...
문제는 최종 결과집합이 1909건으로 테이블 필터조건인 자동로빙여부를 체크하는 과정에서 대부분 걸러졌다.
...
한 블록에 담을 수 있는 로우 수가 줄면, 리프블록이 증가하고 인덱스 높이도 증가한다.
...
할인구분코드가 A~C에 속한 구간을 모두 스캔하면서 나머지 상품코드, 업체코드, 상품유형코드 조건을 필터링하기 때문이다.
...
45. INDEX RANGE SCAN으로 처리되는 쿼리를 INDEX SKIP SCAN으로 유도했을때 성능 효과
...
d. 주문일자 + 상품ID + 고객번호
=> 상품ID 조건이 필터 방식으로 풀리면, 인덱스에서 하루치 주문상세를 모두 스캔하면서 상품ID와 고객번호를 필터링해야한다.
...
- 튜닝방안을 인덱스 추가를 통한 튜닝과 SQL 변경을 통한 튜닝 측면에서 각각작성하시오
...
(
    ...
    ,NVL(SUM(CASE WHEN 투입인출구분코드 = 'G' THEN 정산이자 END), 0) S_정산이자
  FROM 거래
  WHERE 증서번호 = :증서번호
  AND 이체사유발생일자 <= :일자
...
=> 123번 고객을 찾기위해 :Cust_id에 123을 입력하면, 고객번호가 123으로 시작하는 모든 고객이 출력된다.
...
d. 고객ID가 문자형 컬럼이고 고객ID가 인덱스 선두 컬럼이면 INDEX RANGE SCAN이 가능하므로 매우 효율적이다.
...
=>
각 sql에 맞는 최적 인덱스를 각각 생성하면 저장성능이 나빠진다.
...
=>
sql2와 sql3에는 최적이지만 sql1에서 비효율이 발생한다. 특정고객의 3개월치 거래만 조회하면되는데 특정 고객의 모든 거래를 스캔하면서 거래일자를 필터링하기 때문이다.

c.
x1: 고객번호 + 거래일자 + 상품번호
x2: 거래일자
=>
sql1에는 x1이 최적이고, sql3에는 x3이 최적이다. sql2의 경우 상품번호가 필터 조건이 되므로 스캔 비효율이 존재한다. 하지만 특정 고객의 특정 거래일의 레코드가 많지 않을 것으로 예상되므로 비효율이 크지않다.

d.
x1. 거래일자 + 고객번호 + 상품번호
=> 고객번호와 상품번호가 필터조건이되므로 SQL1과 SQL2에 대한 비효율이 발생한다.
...
위 쿼리에서 입력일자는 청약일자와 상관도가 높고 청약일자 조건을 만족하는 데이터는 대부분 입력일자 조건으로 만족할 것이므로 넣지않아도 된다.
...
x2는 결제일자 단일 조건으로 검색하거나, 결제일자와 관리지점번호를 검색할 때 사용된다.
...
참고적으로, = 조건으로 비교하는 선두 컬럼에 해당하는 데이터가 매우 적다면, 오랜 과거 시점이력을 조회하더라도 랜덤 액세스 발생량이 많지 않으므로 의도적으로 시작일자를 제외할 순 있다.
...
브랜치가 2레벨이므로 리프 블록을 포함해 인덱스는 총 3레벨이다.
...
인덱스를 고객ID+연락처구분+종료일자+시작일자(X02) 순으로 구성하면 연락처구분을 IN-List Iterator 방식으로 처리한다.

고객ID+종료일자+시작일자+연락처구분 순으로 구성한 X01 보다 다섯 개 더 많은 블록 I/O가 발생한다.
