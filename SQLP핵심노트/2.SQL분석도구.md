...
3. 다음 중 오라클 예상 실행계획에서 확인할 수 있는 정보가 아닌 것은
...
오라클 예상 실행계획에서 기본적으로 (typical) 확인할 수 있는 정보
...
DBMS_XPLAN.DISPLAY의 매개변수로 typical 대신 alias, outline, advanced 등을 지정하면, QUERY BLOCK 및 OBJECT ALIAS 정보, OUTLINE 정보, COLUMN PROJECT 정보를 추가로 확인할 수 있다.

4. ORACLE AUTOTRACE를 통해 확인할 수 있는 정보

```sql
set autotrace tranceonly;
select empno, deptno from emp where ename = 'SCOTT';
```
...
5. ORACLE AUTOTRACE에서 실제 수행하지 않고 예상 실행계획만 출력하는 옵션

```sql
set autotrace TRACE ONRY EXPLAIN
select * from emp where ename = 'SCOTT';
```

- SET AUTOTRACE ON : SQL실행 + 결과집합 + 예상 실행계획 + 실행통계
- SET AUTOTRACE ON EXPLAIN : SQL실행 + 결과집합 + 실행계획
- SET AUTOTRACE ON STATISTICS : SQL실행 + 결과집합 + 실행통계
- SET AUTOTRACE TRACEONLY : SQL실행 + 예상 실행계획 + 실행통계
- SET AUTOTRACE TRACEONLY EXPLAIN : SQL실행X + 예상실행계획
- SET AUTOTRACE TRACEONLY STATISTICS : SQL실행 + 실행통계

ON은 전부 실행 및 결과출력
Traceonly는 전부 결과는 출력하지않음.
TRACEONLY EXPLAIN은 실행조차 하지않고 실행계획만 출력
TRACEONLY STATISTICS는 실행은 하되 실행통계만출력
...
7. 오라클 트레이스 파일을 분석해 리포트 파일을 생성해주는 명령어.
...
8. 오라클의 기본 SQL 트레이스에서 확인할 수 있는 정보가 아닌 것.
...
b.recursive calls = parse call countss
=> RECURSIVE CALL은 하드파싱 과정에서 딕셔너리 조회 및 DB저장형 함수에 내장된 SQL 수행시 발생한 CALL을 의미한다.
...
10. 힌트를 통한 SQL 트레이스 정보 출력

```sql
select /*+GATHER_PLAN_STATISTICS*/ count(*) from big_table;
select * from table(DBMS_XPLAN.DISPLAY_CURSOR(null,null,'allstarts last'))
```

---

GATHER_PLAN_STATISTICS를 지정하면, SQL 트레이스 정보를 서버 파일이 아닌 SGA 메모리에 기록한다.
...
11. 오라클 SGA 메모리에 기록한 SQL 트레이스 정보 출력
...
12. DBMS_XPAN.DISPLAY_CURSOR 함수로 SQL 트레이스 정보를 출력하기 전에 처리해야할 사항과 관련이 없는 것은
...
13. DBMS_XPLAN.DISPLAY_CURSOR SQL 트레이스 정보와 파일 SQL 트레이스 항목 비교

a. E-ROWS = rows
b. Starts = time
c. Buffers = current + pr
d. Reads = pr
...
16. 대기 이벤트(Wait Event)에 대한 설명으로 부적절한 것.
...
대기 이벤트가 나타나는 대표적인 예는 SGA 공유 메모리에서 특정 자원을 엑세스하려고 래치를 획득하는 과정에서 다른 프로세스와 경합이 발생하거나 디스크로부터 블록 I/O를 요청하거나 클라이언트로부터 다음 작업 요청이 오기를 기다리는 경우다.

17. 하드 파싱과 가장 관련 깊은 대기 이벤트
...
RESPONSE TIME ANALYSIS(응답 시간 분석) 성능관리 방법론은 데이터베이스의 응답시간을 아래와 같이 정의한다.

```
RESPONSE TIME = SERVICE TIME + WAIT TIME = CPU TIME - QUEUE TIME
```

20. 오라클 ARW(AUTOMATIC WORKLOAD REPOSITORY)에 대한 설명으로 가장 올바른 것.

a. 응답시간분석방법론의 단점을 보안하기 위해 Ratio 기반 성능 분석 방법론을 도입했다.
=> ARW는 RATIO 기반 성능 분석 방법론에 응답시간 분석 방법론을 더한 STATSPACK을 확장 및 업그레이드해서 만든 것이다.

b. 오라클 9i 버전까지 사용하던 성능관리 패키지 Startpack을 업그레이드해서 개발했다.

c. AWR을 활용해 효과적으로 문제점을 찾아내려면 성능 이슈가 발생했던 시점을 전후해 가능한한 긴 스냅샷 구간에 대한 리포트를 출력해서 분석해야한다.
=> STATSPACK 방식의 메커니즘이다.

d. 성능 자료를 수집해서 AWR에 저장할 때 'dba*hist*'로 시작하는 각종 뷰를 이용한다.
=> 'dba_hist'로 시작하는 뷰는 사용자 조회용이다.

---

오라클 ARW는 응답 시간 분석 방법론을 지원하는 ORACLE 표준도구다.
...
24. 장애원인 분석을 위해, 평소보다 I/P 관련 대기 이벤트가 높아지는 AWR 동적 성능 뷰
