...
1. 소트 수행 과정에 대한 설명으로 적절한 것

a. Sort Area 공간이 찰 때마다 디스크에 임시저장한 데이터들을 'Sort Runs'라고 한다.

b. 정렬 대상 집합을 디스크에 한 번만 기록하고 작업을 마치는 것을 'Optimal 소트'라고 한다.

=> 정렬 대상 집합이 크지 않아 디스크에 기록하지 않고 PGA의 Sort Area 내에서 작업을 마무리 하는 것을 Optimal Sort라고 한다.

c. 실행계획에 소트 관련 오퍼레이션이 단 한 번 나타나는 것을 'Onepass 소트'라고 한다.

=> 실행계획에 나타난 하나의 소트 오퍼레이션에 대해 디스크에 정렬 대상 집합을 한 번만 기록하고 작업을 마치는 것을 onpass 소트라고한다.

d. 실행계획에 소트 관련 오퍼레이션이 두 번 이상 나타나는 것을 'Multipass 소트'라고 한다.

=> 실행계획에 나타난 하나의 소트 오퍼레이션에 대해 디스크에 정렬 대상 집합을 여러 번 기록하고 작업을 마치는 것을 multipass 소트라고한다.

---

정렬할 대상 집합을 SGA 버퍼캐시를 통해 읽어들이고, 일차적으로 SORT AREA에서 정렬을 시도한다.

PGA 메모리 공간에 할당한 SORT AREA에서 정렬 작업을 완료하는 것이 최적(Optimal)이지만 데이터의 양이 많을때는 정렬된 중간집합을 Temp 테이블스페이스에 임시 세그먼트를 만들어 저장한다.

SORTAREA가 찰 때마다 TEMP 영역에 저장해 둔 중간 단계의 집합을 sort runs라고 부른다.

정렬된 결과를 temp 영역에 임시 저장했다가 다시 읽어들이는 '디스크 소트'가 발생하는 순간 SQL 성능이 크게 나빠진다.

정렬 대상 집합이 크지 않아 SORT AREA 내에서 작업을 마무리하는 것을 optimal(최적) 소트라고 한다.

실행계획에 나타난 하나의 소트 오퍼레이션에 대해 정렬 대상 집합을 한 번만 기록하고 작업을 마치는 것을 onepass 소트, 디스크에 여러 번 기록하는 것을 multipass 소트 라고 한다.

2. AutoTrace를 통한 소트 오퍼레이션 분석에 대한 것으로 부적절한 것

a. 'sorts(memory)' 항목이 1이고, 'sort(disk)' 항목이 1이면, 하나의 소트 오퍼레이션에 대해 Sort Area에서 처리하다가 공간이 부족해 Temp 테이블스페이스를 활용한 것이다.

=> 하나의 Sort Area에서 처리하다가 공간이 부족해 Temp 테이블스페이스를 활용한 경우 sort(memory)와 Sort(Dist) 항목에 각각 1이 추가된다.

b. 'sorts(memory)' 항목이 1이고, 'sort(disk)' 항목이 1이면, 두 개의 소트 오퍼레이션 중 하나는 sort Area에서 처리를 완료하고, 다른 하나는 Temp 테이블스페이스를 활용한 것이다.

c. Group by 또는 중복 제거 연산에 Hash 알고리즘을 사용하면 sorts(memory) 항목과 sort (disk) 항목 모두 0으로 표시된다.

d. 실행계획에 소트 관련 오퍼레이션이 전혀 없으면 sorts(memory) 항목과 sort(disk) 항목 모두 0으로 표시된다.
...
실행계획에 나타난 소트 오퍼레이션의 수는 실행통계(STATISTICS)의 sorts(memory) 항목과 sorts(memory) 항목의 수치를 더한 값과 일치한다.

위 실행계획에서는 소트 오퍼레이션이 두 번(sort order by, window sort) 나타났는데 그 중 한 번은 메모리 내에서 처리를 완료해(sort memory)로 표시됐고, 다른 한 번은 디스크 소트가 발생했다

소트해야 할 데이터양을 감안할 때, 전자는 SORT ORDER BY( 총거래금액이 10만 이상인 데이터만 정렬) 가 해당하고 후자는 WINDOW SORT(전체 거래 데이터에 대한 윈도우함수) 에 해당한다.

GROUP BY 또는 중복 제거 연산에 HASH 알고리즘을 사용하면 sorts memory 항목과 sort dist 항목이 모두 0으로 표시된다.

3. 소트 과정에서 disk I/O가 발생할때 나타나는 대기이벤트

a. direct path write
b. direct path read temp
=> TEMP TABLESPACE에 읽고 쓸때 Direct Path I/O 방식을 사용하는데 이 과정에서 I/O Call이 완료될때까지 발생하는 대기 이벤트
c. direct path sync
d. db file parallel write

---

데이터를 정렬할 때 일차적으로 PGA 메모리에 할당되는 SORT AREA를 이용한다.

SORT AREA 가 부족해지면 TEMP 테이블스페이스를 이용하는데, SORT AREA에 정렬된 데이터를 TEMP 테이블스페이스에 쓰고 이를 다시 읽어 들일 때 DIRECT PATH I/O 방식을 사용한다.

이 과정에서 i/o call이 완료될때 까지 대기가 발생하는데 'direct path write temp / direct path read temp' 이벤트로 측정된다.

4. DISTINCT 시 발생하는 operation
...
-- SELECT STATEMENT
--  SORT (GROUP BY)
--    PARTITON RANGE SINGLE
--      TABLE ACCESS FULL OF '주문'
```

=> sort (group by) 는 group by를 처리할 때 나타나는 오퍼레이션이다. partition range single 오퍼레이션을 통해 주문 테이블이 파티셔닝돼 있음을 알 수 있다
...
=> sort(unique) 는 distinct 또는 unique 연산을 처리할 때 나타난다. Order by 가 있으므로 Hash Unique가 아닌 Sort Unique 오퍼레이션이 나타났다.
...
5. 메모리나 디스크 자원을 가장 많이 사용하는 SORT 관련 연산

CUST 테이블에 대한 조건절은 하나도 없는 상황이다.

```sql
-- A
-- SELECT STATEMENT
--  SORT (AGGREATE)
--    TABLE ACCESS FULL OF CUST
```

=> sort(aggregate) 는 전체 데이터에 대한 count, avg, sum 등 집계함수를 구할 때 나타난다. sort area를 가장 적게 사용하는 연산이다.
...
=> sort(group by)는 group by 할때 나타나는 오퍼레이션이다.
전체 데이터를 정렬하지는 않고 읽는 레코드마다 sort 알고리즘을 이용해 값을 찾아가서 count, sum, min, max 연산을 수행한다.
따라서 결과집합 건수 만큼만의 sort area 를 사용한다.
...
=> hash(group by)는 sort 알고리즘 대신 Hash 알고리즘을 사용한다는 점만 다를 뿐 group by의 결과집함 만큼의 공간만 사용한다.

6. 인덱스로 소트 연산을 대체할 수 없는 실행계획
...
sort aggregate는 조건절을 만족하는 전체 데이터에 대한 count, sum, min, max 값을 구할 때 나타난다.

Count, sum을 구할 때는 어쩔 수 없지만, Min, max를 구할 때는 인덱스 구성을 통해 소트 연산을 대체할 수 있다.

조건절을 만족하는 MIN OR MAX 값을 인덱스에서 한 건만 읽어서 반환할 때 나타나는 실행계획은 다음과 같다.

```sql
--    SELECT STATEMENT
--      SORT AGGREGATE
--        FIRST ROW
--          INDEX RANGE SCAN (MIN/MAX) EMP_X1
```
...
SORT UNIQUE/UNION-ALL 은 UNION 집합 연산자를 사용할 때 나타난다.
UNION 위쪽과 아래쪽 집합 간 중복을 제거하려면 데이터를 모두 읽어야 하므로 인덱스로 소트 연산을 대체할 수 없다.

7. union을 union all로 대체하여도 결과집합이 변하지 않는 sql
...
결제수단코드 조건이 다르고, SELECT-LIST에 PK 컬럼인 결제번호를 포함하므로 상호 배타적이다. 즉, 위아래 집합 간 중복이 없다.
SELECT-LIST에 PK 컬럼을 포함하면 각 집합 내에서도 중복은 없다.
...
같은 주문번호에 대해 결제번호와 결제수단코드만 다르고 나머지 값들이 동일하다면 중복이 발생할 수 있다.
...
주문일자가 다르므로 두 집합간에는 중복이 없지만, 각 집합 내에서 중복이 발생할 수 있다.
...
union을 union all로 변경하면, 2021년 3월 16일에 결제된 데이터는 중복해서 출력된다.

결제일자와 주문일자 조건은 상호배타적 조건이 아니기 때문이다.

부분범위 처리를 가능케하기위해선 소트 연산이 일어나지 않아야한다.

Union 은 중복 레코드를 걸러내기위해 전체 레코드를 읽는 소트연산이 발생하므로 이를 피하기 위해서는 union all 을 사용하면서도 데이터의 중복을 피해야한다.
union all 아래쪽에 결제일자가 20210316인 데이터가 출력되지 않도록 조건절을 추가한다.

결제일자가 null 허용 컬럼일 경우에 대비해 is null 조건절도 추가한다.
...
a.종목코드 + 거래지점코드 + 거래일시 + 거래순번
=> 소트 생략이 불가능한 인덱스 구성이다. 조건절을 만족하는 전체 데이터만큼 인덱스에서 스캔하면서 테이블 액세스하고 정렬까지마친 후 50건을 출력한다. 따라서 전체범위를 처리하는 과정에서 많은 I/O가 발생한다.

b.거래지점코드 + 종목코드 + 거래일시 + 거래순번
=> 소트 생략이 불가능한 인덱스 구성이다. 조건절을 만족하는 전체 데이터만큼 인덱스에서 스캔하면서 테이블 액세스하고 정렬까지마친 후 50건을 출력한다. 따라서 전체범위를 처리하는 과정에서 많은 I/O가 발생한다.

c.종목코드 + 거래일시 + 거래순번 + 거래지점코드

d.종목코드 + 거래일시 + 거래순번
=> 소트생략은 가능하지만 거래지점코드 조건을 만족하는 데이터에 대해 테이블 액세스가 발생한다.

10. SQL을 실행할 때 I/O가 가장 많이 발생하는 인덱스 구성
...
a.접촉구분코드 + 상담일자 + 상담순번
=> 소트를 생략할 수 있지만 상담결과코드를 필터링하기 위한 테이블 랜덤 엑세스가 많이 발생한다.
상담결과코드의 선택도가 3%이므로 평균적으로 테이블을 100번 액세스해야 3개 레코드를 얻는다.
100개 레코드를 출력하려면 평균 3,334번의 액세스를 해애한다.

b.접촉구분코드 + 상담결과코드 + 상담일자
c.상담결과코드 + 접촉구분코드 + 상담일자
d.접촉구분코드 + 상담일자 + 상담결과코드

=> b~d는 소트를 생략할 수 없지만, 조건절을 만족하는 데이터는 평균 300건에 불과하다.
100개를 출력하기 위해 300건을 읽는 비효율이 있지만, 테이블 랜덤 액세스는 최대 300번 정도 발생한다.

d보다는 b,c가 인덱스 스캔과정에서 읽는 블록수가 적다.

11. 소트 연산을 생략하면서 I/O를 최소화할 수 있는 인덱스 구성
...
a. 상담원ID + 상담결과코드 + 상담일자
=> 소트연산은 생략이 가능하지만 상담결과코드가 인덱스 필터조건으로 작동하므로 많은 I/O가 발생한다.

b. 상담결과코드 + 상담일자 + 상담원ID
=> 소트연산을 생략할 수 없음

c. 상담결과코드 + 상담원ID + 상담일자
=> 소트 생략이 가능하며, 상담일자를 인덱스에서 필터링하므로 불필요한 테이블 엑세스 발생하지않는다.
상담일자 조건이 전월 이전에 상담한 데이터이므로 대부분의 상담 데이터가 조건을 만족한다.
따라서 인덱스 스캔 비효율도 거의 없다.

d. GROUP BY 소트 연산을 생략할 수 없다.
...
상품유형코드의 선택도가 10%(1000건 레코드의 카디널리티가 100)이므로 조인 후 최근 1년간 계약건수 500만 건 중 50만건을 읽을 것으로 예상된다.

DISTINCT 연산을 통해 중복을 제거한 최종 결과집합은 100개의 상품 집합인데, 문제에서 제시한 실행계획대로 읽으면 50만 건의 계약 데이터를 랜덤 엑세스 방식으로 읽어야 한다.

EXISTS 서브쿼리로 변환하면 100개 상품에 대해 계약과 조인에 성공하는 데이터가 존재하는지만 확인하면 되므로 훨씬 효율적이다.
...
-- 계약_x2(상품번호) =>  계약일자_x2(상품번호 + 계약일자) 로  컬럼추가
...
EXISTS 서브쿼리로 변환했는데 옵티마이저가 서브쿼리를 UNNESTING 한 후에 해시 세미조인하거나 계약 테이블을 먼저 읽어서 상품과 조인하는 방식으로 수행하면 원하는 성능 효과를 얻을 수 없다.
...
서브쿼리를 unnesting 하지 못하도록 no_unnesting 만 추가해도 항상 필터방식으로 실행된다. 필터방식은 nl 세미 조인과 사실상 차이가 없고 항상 메인 쿼리를 먼저 읽으므로 Leading 또는 ordered 힌트도 불필요하다.
...
-- 주문상품_pk : 주문번호 + 상품코드
-- 평균 주문 하나당 2건의 주문상품
-- 연간 1200만건이면 평균 2400만건
-- 모든 상품에 대한 주문이 골고루 발생하고 상품이 100개 이므로
-- 특정 상품코드에 대한 주문상품 200만건
...
주문과 주문상품을 조인한 후 DISTINCT 연산을 통해 중복을 제거한 최종 결과집합은 주문 단위 집합이다.

Exists 서브쿼리로의 변환을 생각해볼 수 있지만 주문당 평균 주문 상품은 2건에 불과하므로
데이터 존재 여부만 확인한다고 해서 얻는 이점은 별로 없다. 조인할 레코드의 수가 절반 정도로 줄어드는 효과만 있다.

1년치 주문이 1200만 건이므로 랜덤 엑세스 방식으로 주문상품을 필터링하기는 부담스럽다.
쿼리를 그대로 둔 상태에서 Nl 조인으로 처리하면 랜덤 엑세스 및 소트 부하가 더 크다.

결과적으로 해시 조인이 가장 효과적이다.

100개의 상품에 골고루 주문이 발생한다고 했으므로 상품코드 조건을 만족하는 주문 상품 데이터는 평균 200만건이다.
주문번호에 대한 중복이 존재하지 않는다. 주문상품\_pk가 주문번호 \* 상품코드 이기때문이다.
...
200만 건도 많은 양이고 버퍼캐시 히트율이 낮을 때 Single Bock I/O 방식으로 많은 디스크 블록을 읽는다면 성능이 매우 느릴수 있다. 게다가 쿼리 수행빈도가 높지 않다면 인덱스를 추가하는 것도 부담스럽다.
...
해시 조인으로는 소트연산을 생략할 수 없으므로 NL조인으로 유도해야 한다.
정렬 기준의 소트를 생략하려면 인데스를 지점id\_계약일시 로 구성해야 한다.
...
--    VIEW
--        NESTED LOOPS
--          TABLE ACCESS BY INDEX ROWID 계약
--            INDEX RANGE SCAN OF 계약_x1
--          TABLE ACCESS FULL OF 상품
```

15. 성능 측면에서 가장 효과적인 페이징처리 패턴
...
TOP N STOPKEY 알고리즘이 작동할 수 있어야하는데 이를 위해서는 인라인 뷰에 안쪽 order by를 명시하고 바로 바깥 쪽에 ROWNUM <= 조건을 명시해야 한다.
...
WHERE CEIL(ROWNUM / 10) = :PAGE
```

16. 집계함수 MAX를 활용한 최적쿼리
...
AND 변경일시 = (
  SELECT MAX(변경일시)
  FROM 상품변경이력
  WHERE 변경일시 >= TRUNC(ADD_MONTHS(SYSDATE, -12)
)
...
현재는 상품번호 + 변경일시 + 변경구분코드 순으로 구성된 pk 밖에 없다.
...
A번은 틀린 결과를 출력하고 B번은 같은 데이터를 2번 읽는 비효율이 있다.

D번은 인덱스 구성이 변경되면 틀린 결과를 반환할 가능성이 있으므로 사용하지 않는 것이 좋다.
...
위 쿼리는 가장 단순하게 원하는 데이터를 찾는 쿼리이다.

하지만 성능 측면에서 비효율이 생길 수 이다.

인데스에서 최소 또는 최대값을 빠르게 찾으려면, 조건절 컬럼과 min, max 함수 인자 컬림이 모두 인덱스에 포함되어있어야 한다. 즉, 테이블 액세스가 전혀 발생하지 않아야 한다.

하지만 변경구분코드를 필터링하려면 테이블 엑세스해야한다.

결국, first orw (min/max) 기능이 작동하지 않아 조건절 데이터를 모두 읽어야 하고 상품조건에 대한 변경이력이 매우 많다면 성능에 문제가 될 수 있다.

인덱스를 변경하거나 추가할 수 없다면 top1 쿼리로 작성하는 것이 좋다
...
from (
  ...
  rownum() over(partition by 상품번호 order by 변경일시 desc) no
  from 상품변경이력
  where 변경일시 >= to_date('20210301')
  and 변경일시 < to_date('20210401')
)
where no = 1
AND 변경구분코드 = 'C2'
```

2021년 3월에 변경된 상품변경이력 중에서 상품별 최종 데이터를 찾는 방법은 위와 같다.
...
위 SQL로 요건에 만족하는 결과를 구할 수 있지만, 상품할인율을 두 번 조회하므로 최적은 아니다.

```sql
select 기준일자, 할인율
from (
  select 기준일자, 할인율, row_num() over (partition by 기준일자 order by 변경순번 desc)
  from 상품할인율
  where 상품번호 = 'R0014'
  and 변경일시 >= to_date('20210301')
  and 변경일시 < to_date('20210401')
...
a. 상품번호 + 변경구분코드 + 변경일자 + 변경일련번호
b. 변경구분코드 + 상품번호 + 변경일자 + 변경일련변호
c. 상품번호 + 변경일자 + 변경일련번호 + 변경구분코드
d. 변경일자 + 변경일련번호 + 상품번호 + 변경구분코드
...
3. = 연산자가 아닌 조건절 컬러믄 데이터 분포를 고려해 추가 여부 결정

In 조건절에 쓰인 컬럼인 변경구분코드를 인덱스 앞쪽에 두어도 소트연산을 생략할 수 있을까?

IN문은 등치조건이 아니다. IN-LIST ITERATOR 방식으로 풀려야 등치조건이 된다.

하지만 IN-LIST ITERATOR 방식으로 풀린다 하더라도 union all로 결합한 두 집합의 정렬 순서를 
인덱스로 제어할 방법은 없다.

결론적으로 소트연산을 생략하려면 in-list-iterator 방식으로 풀려선 안된다.

즉, IN-LIST 조건절을 인덱스 엑세스 조건으로 사용하면 안되고 필터조건으로 사용되야 한다.

따라서 변경견구분코드는 인덱스에서 아에 졔외 되거나 맨 뒤쪽에 둔 상태에서 소트 생략 가능한 구성을 고민해야 한다.

22. 서브쿼리 내의 인라인 뷰에서 메인쿼리 테이블을 참조하는 방법
...
AND (H.변경일자, H.변경순번) = (
  ...
  (SELECT  변경일자, 변경순번
    FROM 상태변경이력
    WHERE 장비번호 = P.장비번호
    ORDER BY B.변경일자 DESC, B.변경순번 DESC
  )
  WHERE ROWNUM <= 1
)
...
따라서 TOP N 패턴의 성능 이점을 자유롭게 활용할 수 있다.

# DML 튜닝

23. DML 성능에 영향을 미치지 않는 요소

a.LOCK

b.옵티마이저모드
=> DML의 경우 옵티마이저 모드는 항상 ALL_ROWS(전체 처리속도 최적화)로 작동한다.
...
24. 에플리케이션에서 다수의 commit시 성능이 저하되는 이유
...
a.DB CALL이 많이 발생
b.REDO 로그 버퍼를 로그 파일에 기록
c.DBWR가 변경된 버퍼블록을 데이터파일에 기록
=> 서버 프로세스가 변경한 버퍼블록들이 커밋 시점에 바로 데이터 파일에 기록되지 않는다.
추후에 DBWR에 의해 일괄적으로 기록된다. 커밋시점에 redo 로그를 디스크에 기록했기 때문이다. 
버퍼블록 내용이 유실되더라도 트랜잭션의 영속성은 보장된다.
d.서버 프로세스가 LGWR의 직업 완료 신호를 받을때까지 대기
...
25. 온라인 트랜잭션이 없을때 동작하는 UPDATE BATCH 프로그램의 성능 개선방법과 거리가 먼 것.

a.nologging 옵션을 활용한다.
=> nologging은 insert 시만 가능하다.
b.update 되는 컬럼을 포함한 인덱스를 unusable 상태로 변경하고, 작업 완료 후에 재생성한다.
c.PK,FK 등 제약을 해제하고, 작업 완료 후에 재설정한다.
d.병렬처리를 활용한다.

26. 온라인 트랜잭션이 존재하는 주간에 대량데이터를 일괄 INSERT BATCH 성능개선 방안으로 적절한 것.

a.Direct Path Insert 기능을 활용한다.

=> 온라인 트랜잭션이 존재하는 주간에 대량 데이터를 일괄 INSERT 하는 배치 프로그램의 경우
DIRECT PATH INSERT 기능을 사용하면 'EXCLUSIVE MODE TM LOCK'을 설정하므로 다른 트랜잭션이 해당 테이블에 DML을 수행하지 못하게 된다. 트랜잭션이 빈번한 주간에 이 옵션을 사용하는 것은 금물이다.

b.nologging 옵션을 활용한다.
=> INSERT에 nologging 옵션을 활용할 수 있지만, 이는 DIRECT PATH INSERT 할 때만 작동한다.

c.인덱스 및 제약을 해재한 상태에서 INSERT 한다.
=> 온라인 트랜잭션이 수행되는 도중에 인덱스 및 제약을 해제할 수는 없다.

d.ARRAY PROCESSING 기능을 활용한다.

27. DIRECT PATH INSERT가 빠른이유로 부적절한 것.

a. UNDO 데이터를 남기지 않는다.
b. 테이블에 LOCK을 설정하지 않는다.
=> DIRECT PATH INSERT 시, 'DIRECT PATH INSERT EXCLUSIVE MODE TM LOCK'을 설정하므로 다른 트랜재션이 해당 테이블에 DML 하지 못하게 된다.
c. freelist를 참조하지 않고 HWM 바깥 영역에 데이터를 순차적으로 입력한다.
d. insert 할 블록을 버퍼캐시에서 탐색하지 않고 데이터파일에 직접 기록한다.

28. DIRECT PATH INSERT가 작동하는 경우

a. INSERT-SELECT 문에 APPEND 힌트 사용
b. INSERT-SELECT 문을 수행하기 전에 테이블을 nologging 모드로 전환
=> nologging 모드는 redo 로그 생성하지 않게 하는 옵션이므로 direct path insert와 직접적인 관련이 없다.
다만, redo 로그를 생성하지 않는 기능은 테이블을 nologging 모드로 전환한 상태에서 direct path insert로 데이터를 입력 할 때만 작동한다.
c. 병렬 DML을 활성화한 상태에서 INSERT 문에 Parallel 힌트 사용
d. CTAS(create table ... as select) 문으로 테이블 생성

29. INSERT문 성능개선

```sql
-- DB로그는 Archive 모드이며, 보기 항목에 기술한 명령문 외에 다른 옵션이나 설정은 전혀 활성화 하지 않는 상태
...
=> NOLOGGING 힌트는 제공되지 않으므로 성능 향상에 전혀 도움이 되지 않는다.
...
=> INSERT 할 테이블 뒤쪽에 지정한 NOLOGGING은 기능과 무관하다.
REDO 로그를 생략하지는 못했지만 APPEND 힌트를 통해 DIRECT PATH INSERT 기능은 작동하므로 성능 개선에 도움이 된다.
...
테이블을 NOLOGGING 모드로 전환했으므로 REDO 로깅을 생략하면서 Direct Path Insert 기능까지 작동하여 성능이 크개 개선된다.
...
=> 병렬로 INSERT 할 때는 APPEND 힌트를 지정하지 않아도 Direct Path Insert 방식으로 작동한다.
단, 사전에 'ALTER SESSION ENABLE PARALLEL DML' 명령을 통해 병령 DML 을 활성화해야한다.
병렬 DML을 활성화한 후 NO LOGGING 방식으로 INSERT 하면 가장 성능이 좋지만 병렬 DML을 활성화하지 않았다.

30. 온라인 트랜잭션이 없는 상황에서의 BATCH INSERT 성능개선
...
a.테이블을 nologging 모드로 전환한 상태에서 INSERT 문에 append 힌트를 사용한다.

b.병렬 DML을 활성화한 후 INSERT 문에 parallel 힌트를 사용한다.

c.인덱스 및 계약을 해제한 상태에서 INSERT를 수행한다.

d.Array Processing을 활용해 10만 건 단위로 INSERT 하도록 프로그램을 수정한다.

=> ARRAY PROCESSING은 성능 개선에 큰 효과가 있지만 INSERT INTO SELECT 보다 느리다.

31. 온라인 트랜잭션이 없는 상황에서 배치 INSERT 성능개선

```sql
-- TARGET_T 테이블에 PK 인덱스만 존재하며, 다른 인덱스는 없다.
...
delete 문을 사용하면 redo로그와 undo 를 생성하면서 레코드 단위로 삭제하므로 데이터가 많을 때 상당히 오랜 시간이 소요된다.

```sql
TRUNCATE TARGET_T;
```

=> TRUNCATE 문을 사용하면 딕셔너리 상에서 익스텐트만 반환하는 방식을 사용하므로 대량 데이터도 빠르게 삭제할 수 있다.

```sql
ALTER TABLE TARGET_T MODIFY CONSTRAINT TARGET_T_PK DISABLE DROP INDEX;
```

=> DML중에 제약 조건을 체크하고 인덱스를 관리하는 부하가 매우 크므로 작업 전에 제약을 해제하고 인덱스를 unusable 상태로 변경하면 성능을 크게 높인다.
...
=> SOURCE_T는 병렬로 읽지만 TARGET_T 테이블에 INSERT 할때는 QC 혼자서 처리한다.
병렬 DML을 활성화했지만, INSERT 문에 parallel 힌트를 사용하지 않았기때문이다.
제대로 병렬처리를 하려면 insert 문에도 parallel 힌트를 추가해야 한다.

```sql
ALTER TABLE TARGET_T NOLOGGING;
```

target_t 테이블을 nologging 모드로 전환하면 redo 로깅을 생략할 수 있어 더 빠르게 insert 할 수 있다.

```sql
ALTER TABLE TARGET_T MODIFY CONSTRAINT TARGET_T_PK ENABLE NOVALIDATE;

ALTER TABLE TARGET_T LOGGING;
ALTER SESSION DISTABLE DML;
```

=> DML이 끝난 후 순차적으로 인덱스 활성화, LOGGING 모드, DML 병렬처리 비활성화를 진행한다.

```SQL
TRUNCATE TABLE TARGET_T;

ALTER TABLE TARGET_T MODIFY CONSTRAINT TARGET_T_PK DROP INDEX;

ALTER SESSION ENABLE PARALLEL DML;

ALTER TABLE TARGET_T NOLOGGING;

INSERT /*+APPEND PARALLEL(T1 4)*/ INTO TARGET_T T1
SELECT /*+FULL(T2) PARALLEL(T2 4)*/ *
FROM SOURCE_T T2;

COMMIT;

ALTER TABLE_T MODIFY CONSTRAINT TARGET_T_PK ENABLE NOVALIDATE;
ALTER TABLE TARGET_T LOGGING;
ALTER SESSION DISABLE PARALLEL DNL;
```

32. 온라인 트랜잭션이 없는 환경에서 원격 INSERT~SELECT 배치처리 성능 개선

```sql
-- UPDATE 문에서 C1 조건절을 만족하는 데이터는 90% 이상이다.
...
BEGIN
  ...
  -- 배치 프로그램을 재실행할 경우를 대비하기 위한 DELETE
  ...
  EXCEPTION
    WHEN DUP_VAL_ON_INDEX THEN
      INSERT_LOG(SYSDATE, 'INSERT MYTAB_TEMP', 'FAIL', '중복데이터');
  END;
  /
...
INSERT 후 따로 UPDATE 할 필요없이 mytab_temp 테이블을 생성할 때 아예 변경된 값을 입력하면 된다.
...
Id에 중복값이 존재하는지확인한 후에 mytab 테이블에 입력하도록 프로그램을 수정하면
pk 제약을 생성하지 않아도 된다.
...
NOLOGGING
AS
SELECT C0 AS ID, C1, C2, C3, (CASE WHEN C1 < TRUNC(SYSDATE) C4 + 1 ELSE C4 END)
AS C4
FROM YOURTAB@RDS
WHERE C0 IS NOT NULL
AND C5 > 0;
...
--        INDEX UNIQUE SCAN OF 고객번호_PK
```

위 쿼리는 법정대리인을 등록하지 않은 50% 의 미성년 고객까지 모두 update 하는 문제점이 있다.
...
법정대리인을 등록한 미성년 고객만 update 할 수 있고,
...
고객\_x3 인덱스를 사용하면 full scan 처리 되겠지만, 오라클의 경우 고객\_X3 인덱스에 법정대리인 고객번호를 입력한 데이터만 저장하므로, 불필요한 데이터를 스캔하는 비효율은 없다.

오라클이 아닌 경우, 고객\_X3 인덱스에 모든 고객 데이터를 저장하므로 불필요한 데이터까지 스캔해야하지만, 적어도 불필요한 데이터 액세스는 없다.

오라클 옵티마이저는 법정대리인\_고객번호 is not null 조건을 자동으로 추가해 준다.
...
UPDATE 고객 C
(
  -- 법정대리인이 등록된 미성녀자 고객중 연락처가 법정대리인연락처가 아닌사람.
  SELECT /*+LEADING(C) USE_NL(P) INDEX(C 고객_X3) INDEX(P 고객_PK)*/
  C.법정대리인_연락처 , P.연락처
  FROM 고객 C, 고객 P
  WHERE C.성인여부 = 'N'
  AND C.법정대리인_고객번호 IS NOT NULL -- 옵티마이저가 자동으로 생성함
...
-- ROW LOCK 경합이 자주 발생하며, 평균 UPDATE 건수는 8만건이다.
...
-- 업체코드가 Z이고 가용제고량이 0이고 가상제고수량이 0인 레코드의 품질유지일을 오늘날짜에서 상품제고이력의 최근 변경일자를 뺀 값으로 업데이트
...
스칼라 서브쿼리에서 상품재고 테이블을  상품재고이력과 조인할 이유가 없다.
...
MERGE INTO 상품재고 X
USING (
SELECT /*+LEADING(A) NO_MERGE(B) PUSH_PRED(B)*/
A.상품번호, B.신규_품질유지일
FROM 상품재고 A,
(
...
ON (X.상품번호 = Y.상품번호)
WHEN MATCHED THEN UPDATE SET x.품질유지일 = Y.신규_품절유지일
```
...
35. 데이터베이스 call에 대한 설명

a.같은 SQL을 여러 번 수행할 때 '바인드 변수'를 사용하면 Parse Call은 한 번만 일어난다.
=> 애플리케이션 커서를 캐싱하지 않는 한, parse call은 바인드 변수를 사용해도 매번 일어난다.

b.Parse Call 단계에서 Recursive Call이 발생할 수 있다.
=> parse call 단계에서 하드 파싱을 하게 되면 딕셔너리를 조회하는 과정에 recursive call이 발생한다.

c.DML은 대부분 I/O가 Execute Call 단계에서 일어난다.
=> DML은 parse call을 제외하면 모든 i/o가 execute call 단계에서 일어난다.

d.SELECT 문은 대부분 I/O가 Fetch Call 단계에서 일어난다.
=> select 문은 대부분 i/o가 Fetch call 단계에서 일어난다.

36. 데이터베이스 Call에 대한 올바른 설명

a.Parse Call 단계에서 Recursive Call이 발생할 수 있다.
=> parse call 단게에서 하드 파싱을 하게 되면 딕셔너리를 조회하는 과정에서 Recursive call이 발생한다.

b.Execute Call 단계에서 Recursivce Call이 발생할 수 있다.
...
c.Fetch Call 단계에서는 Recursivce Call이 발생하지 않는다.
...
d.User Call은 네트워크를 경유하므로 Recursivce Call과 비교해 성능에 미치는 영향이 훨씬 더 크다.
...
a.select 문에 대한 트레이스 결과이다.
=> fetch call 이 10000 번 발생했고 블록 i/o와 처리 건수가 모두 fetch 단계에 나타났으므로 select 문에 대한 트레이스 결과다.

b.애플리케이션 커서를 캐싱한 상태에서 SQL을 실행했다.
=> 5000번 실행하는동안 parse call 이 1회 발생한 사실을 통해 애플리케이션 커서를 캐싱한 상태에서 sql을 실행했음을 알 수 있다.

c.Array Size를 10으로 설정한 상태이다.
=> 100000 로우를 읽는 동안 10000번 fetch 했으므로 array size 는 10이다.

d.한 번 실행할 때 평균 4개 블록을 읽었다.
=> 5000번 실행하는 동안(Execute) 블록을 40000 개 읽었으므로 한 번 실행할 때 평균 8개 블록을 읽었다.

38. database call을 줄이는 방법

a. 바인드 변수를 사용한다.
=> 바인드 변수를 사용한다고 데이터베이스 Call이 줄어들지 않는다. 바인드 변수를 사용해도 애플리케이션 커서를 캐싱하지 않는 한 parse call은 매번 일어나기 때문이다.

b. Array Size를 늘린다.
=> ARRAY SIZE를 늘리거나 페이징 처리 기법을 사용하면 FETCH CALL을 줄일 수 있다.

c. Array Processing을 활용한다.
=> ARRAY PROCESSING을 활용하면 execute call을 줄일 수 있다.

d. 전체 결과집합을 한 번에 출려하지 않고 페이징 처리 기법을 활용한다.
...
절차적인 루프 처리에 의해 많은 데이터베이스 Call을 발생시키는 대표적인 사례이다.
ONE SQL로 구현하면 단 한 번의 데이터베이스 call로 처리하므로 성능 개선에 큰 도움이 된다.
...
-- 은행입금내역을 고객ID로 GROUP BY 한 결과는 10만건
...
은행입금내역 테이블에서 입금일시가 '20210329' 인 고객의 고객별 총입금액(10만건)을 가지고
수납 테이블에서 같은 고객의 수납일자가 '20210329' 라면 수납금액을 총입금액으로 업데이트
수납 테이블에 해당 고객이 없으면 insert

문제점

- 입금일시 컬럼을 변환했으므로 인덱스 Range Scan 불가
- 수납일자 조건에 자동 형변환이 발생해 인덱스 Range Scan 불가
- Literal SQL에 의한 하드파싱 부하
- 절차적 루프 처리에 의한 반복적인 데이터베이스 Call
- 루프 내에서 건건이 커밋
...
(
   ...
   from 은행입금내역
   where 고객ID = r.고객ID
   and 입금일시 between to_date('20210329' ,'yyyymmdd')
   and  to_date('20210330' ,'yyyymmdd') - 1/24/60/60
)
where 수납일자 = '20210329';
and exists (
  select 'x'
...
from 은행입금내역 r
where 입금일시 between to_date('20210329' ,'yyyymmdd')
and  to_date('20210330' ,'yyyymmdd') - 1/24/60/60
and not exists (
  select 'x' from 수납
  where 고객ID = r.고객ID
...
41. ARRAY PROCESSING에 대한 설명으로 부적절한 것

a.PL/SQL 프로그램보다 JAVA 프로그램에서 사용하면 성능 개선율이 높다.
=>
ARRAY PROCESSING 이 성능 효과를 발휘하는 핵심 원리는 데이터베이스 call을 줄이는 데 있다.
네트워크를 경유하는 프로그럼은 PL/SQL 프로그램보다 데이터베이스 call 부하가 더 크다.
따라서 JAVA 프로그램에 array processing을 적용했을 때 성능 개선 효과가 더 크다.

b. Array 단위를 늘릴수록 성능은 선형적으로 좋아진다.
=> ARRAY 단위를 늘리면 성능이 좋아지지만 개선율은 점차 감소한다. 일정 수준을 넘어서면 개선 효과는 없고 지원만 많이 사용하므로 적정 크기로 설정해야 한다.

c. 효과를 극대화하려면 연속된 일련의 과정을 모두 Array 단위로 처리해야한다.
=> 세 개의 단위 작업 중 첫 번째와 세 번째에만 ARRAY PROCESSING을 적용하면,
두 번째 작업이 병목구간으로 작용한다.

d. Array Processing이 성능 효과를 발휘하는 핵심 원리는 데이터 베이스 Call을 줄이는 데 있다.

42. FETCH CALL 최소화 방안

a.부분범위 처리를 활용한다.
b.화면 페이징 처리 기법을 활용한다.
c.ArraySize를 늘린다.
d.네트워크 패킷크기를 늘린다.
=> SDU, TDU 등 세션이나 네트워크 프로토콜 레이어에서 사용하는 패킷 크기를 늘리면
서버 / 클라이언트 간 통신 횟수를 줄일 수는 있다.
하지만 이는 오라클이 '한 번의 FETCH CALL' 내에서 전송할 데이터량이 많을 때 일정량씩 나눠 전송하는 단위이므로 FETCH CALL 횟수에는 영향을 주지 않는다.

43. 부분범위처리에 대한 설명으로 옳은 것

a. 부분범위 처리를 활용한 튜닝은 오라클에서만 활용 가능하다.
=> 부분범위 처리를 활용한 튜닝은 오라클뿐만 아니라 데이터를 일정량씩 나눠서 전송하는 모든 dbms에 활용할 수 있다.

b. 한 번의 Fetch Call을 처리하고나면 서버 프로세스는 CPU를 점유한 채 다음 Fetch Call을 기다린다.
=> 한 번의 FETCH CALL을 처리하고 나면 서버 프로세스는 CPU를 os에 반환한 상태에서 다음 fetch call을 기다린다.

c. ArraySize를 작게 설정할수록 부분범위 처리에 대한 응답속도는 빨라진다.

d. 부분범위 처리를 활용할 때는 읽어야 할 데이터 범위에서 조건절을 만족하는 데이터가 적을 수록 빠른응답 속도를 보인다.
=> array size를 작게 설정하거나 읽어야 할 데이터 범위에서 조건절을 만족하는 데이터가 많을 수록 운반 단위를 빨리 채워 응답속도가 빠르다.

---

서버 프로세스가 쿼리 집합을 한 번에 모두 전송하지 않고 클라이언트로 부터 fetch call이 올 때마다 일정량 나눠서 전송하는 것을 '부분범위 처리' 라고 한다.

44. DB I/O 및 네트워크 성능에 대한 부적절한 설명

a. Multiblock I/O 단위를 줄일수록 Full Table Scan 할 때 I/O Call 횟수가 늘어난다.

b. 테이블 익스텐트를 작게 설정하면 FULL TABLE SCAN I/O Call 횟수가 늘어날 수 있다.

c. Array Size를 크게 설정할수록 메모리에서 블록을 읽는 횟수가 늘어난다.
...
d. SDU, TDU등 네트워크 패킷을 작게 설정할수록 서버와 클라이언트 간 통신 횟수가 늘어난다.

45. 쿼리성능개선방안

a. 코드명,상품명 처럼 값이 변하지 않는 이름 속성을 출력할 때, ID 속성은 DB에서 읽고 이름 속성은 애프리케이션 서버 레이어에 미리 케싱해준 데이터 또는 메모리 DB를 조회하여 조인 부하를 줄인다.

b. 코드명, 상품명, 고객명 등을 조회할 때는 조인에 의한 성능 저하를 막기 위해 가급적 DB 저장형 함수를 사용하는 것이 좋다.
=> 코드명, 상품명, 고객명 등을 조회할 때 조인 대신 dB 저장형 함수를 사용하면 db Call 부하로 인한 성능은 오히려 더 느려진다.

c. 인덱스 및 SQL 튜닝만으로 만족스러운 성능을 내기 어려울 때, 값이 변하지 않는 속성은 반정규화 하여 거래 테이블에 미리 저장해주는 방안을 고려할 수 있다.

d. 조회 결과집합을 한 번에 모두 출력하지 않고 페이징 처리를 활용하면 화면에 출력하는 최종 데이터에 대해서만 조인을 구현할 수 있어 성능 개선에 좋다.

46. DB 저장형 사용자정의함수/프로시저에 대한 부적절한 설명

a. 가상머신 사엥서 실행되는 Interpreting 언어이다.

b. 호출 시 마다 SQL 실행엔진과 PL/SQL 가상머신 사이의 Context Switching이 발생한다.

c. 함수에 SQL이 내장돼 있으면 Execute Call이 발생할 때마다 Recursivce Call도 한 번씩 발생한다.

=> 함수에 SQL이 내장돼 있으면, 함수 실행 횟수만큼 Recursive Call이 발생한다.
함수가 SELECT-LIST에 있다면, 결과집합 건수만큼 함수가 발생한다. 함수가 조건절에 있으면 필터처리되는 건수 만큼 함수가 실행된다.

d. 함수/프로시저에 내장된 SQL이 참조하는 테이블에 구조, 인덱스 변경, 통계정보 재수집등이 일어나면, 이후 최초 실행된 시점에 재컴파일이 일어난다.

47. 함수 내장 SELECT 문의 DB CALL 횟수 계산
...
a. 부분범위 처리를 활용하기 위해 ORDER BY를 제거하고, INDEX_DESC(B (게시판코드, 등록일시)) 힌트를 추가한다
...
c. CODE_NM 함수를 아래처럼 스칼라 서브쿼리에 씨워서 사용한다.
...
d. 작성자명과 게시글분류명 조회용 함수를 조인문으로 변경하고, NL 조인으로 유도한다.
=> 함수를 풀어서 조인문으로 변경하는 것이 함수 호출 부하를 줄이는 근본적인 해법이다.
조인으로 변경할 경우, nL 조인이 가장 효과적이다. 부분범위처리가 가장 잘 작동하기 때문이다. 해시 조인은 소트 연산을 생략할 수 없다.

50. 결과집합이 큰 쿼리의 DB 저장형 함수에 내장된 SQL에 대한 RECURSIVE CALL 횟수를 줄이는 방법

스칼라 서브쿼리 - Deterministic 함수 캐싱 - Result Cash - Native Compile
...
51. DB 저장형 함수의 READ CONSISTENCY

a. 함수 결과의 일관성은 기본적으로 보장되지 않지만, Deterministic으로 선언하면 보장된다.

=> deterministic 함수는 FETCH CALL 단위로 캐싱하므로 함수 결과의 일관성을 보장하지 않는다.

b. 함수를 스칼라 서브쿼리에 씌워서 실행하면 캐싱된 데이터를 반환하므로 일관성이 보장된다.

=> 함수를 스칼라 서브쿼리에 실행할 때의 캐싱 효과는 SQL 수행을 마칠 때 까지 유효하므로 일관성을 보장할 것 같지만 그렇지 않다. 각 입력값에 대한 첫 번째 함수 호출이 일어날 때까지의 시간차 때문에 보장이 어렵고 모든 값을 다 캐싱할 수 없다는 점 때문에도 보장하기 어렵다.

c. 코드명, 상품명 등을 조회할 때 함수를 사용하지 않고 스칼라 서브쿼리로 조인하면 일관성이 보장된다.

=> 스칼라 서브쿼리 조인은 일관성을 보장한다. 즉, 중간에 값이 변하더라도 쿼리 시작 지점 기준으로 일관된 데이터를 출력한다.

d. 위 세가지 방안 모두 일관성을 보장하지 않는다.
...
회원\_X01 인덱스를 사용하면 생년월일 LIKE 조건을 만족하는 건수만큼 테이블 엑세스 단계에서 필터링하므로 함수를 5만번 호출한다.

회원\_X02 또는 회원\_X03 인덱스를 사용하면, 생년월일 LIKE 조건은 인덱스 액세스조건이지만 전화번호는 인덱스 필터 조건이므로 인덱스 스캔단계에서 함수를 5만 번 호출한다.

인덱스 컬럼 중 첫 번째 나타나는 범위검색 조건까지가 인덱스 액세스 조건이고 나머지는 필터 조건이다.

반면 회원\_X04 인덱스를 사용하면 전화번호가 액세스조건이므로 함수는 단 1회 호출한다.
