...
=> 실행계획에 나타난 하나의 소트 오퍼레이션에 대해 디스크에 정렬 대상 집합을 여러 번 기록하고 작업을 마치는 것을 onpass 소트라고한다.
...
=> 하나의 Sort Area에서 처리하다가 공간이 부족해 Temp 테이블스페이스를 활용한 경우 Sort Dist 항목에 1이 추가된다.
...
7. union을 union all로 대해 결과집합이 변하지 않는 sql
...
c. 상담결과코드 + 상담원ID + 상담일자
=> 소트 생략이 가능하며, 상담일자를 인덱스에서 필터링하므로 불필요한 테이블 인덱스가 발생하지않는다.
...
소트 연산을 생략하려는 목적이 전체 결과 집합 중 앞쪽 일부를 빨리 출력하기 위함이기 때문이다. 아래와 같이 order by 뿐 아니라 Group by도 인덱스를 장 구성하면 소트 연산을 생략할 수 있다.
...
EXISTS를 서브쿼리로 변환하면 100개 상품에 대해 계약과 조인에 성공하는 데이터가 존재하는지만 확인하면 되므로 훨씬 효율적이다.
...
데이터 존재 여부만 확인한다고 해서 얻는 이점은 별로 없다. 조인할 레코드의 수가 절반 정도로 줄어드는 교화만 있음
...
In 조건절에 쓰인 컬럼인 변경구분코드를 앞쪽에 두어도 소트연산을 생략할 수 있을까?

IN문은 등치조건이 아니다. IN-LIST ITERATOR 방식으로 풀려야 등치조건이 된다.

하지만 IN-LIST ITERATOR 방식으로 풀린다 하더라도 union all로 결합한 두 집합의 정렬 순서를 인덱스로 제어할 방법은 없다.
...
추후에 DBWR에 의해 일괄적으로 기록된다. 커밋시점에 redo 로그를 디스크에 기록했기 때문이다. 버퍼블록 내용이 유실되더라도 트랜잭션의 영속성은 보장된다.
...
25. 온라인 트랜잭션이 없을때 동작하는 UPDATE BATCH 프로그램의 성능 개선방법
...
26. 온라인 트랜잭션이 있을때 INSERT BATCH 성능개선

a.Direct PAth Insert 기능을 활용한다.

=> 온라인 트랜잭션이 존재하는 주간에 대량 데이터를 일괄 INSERT 하는 배치 프로그램의 경우
DIRECT PATH INSERT 기능을 사용하면 EXCLUSIVE MODE TM LOCK을 설정하므로 다른 트랜잭션이 해당 테이블에 DML을 수행하지 못하게 된다. 트랜잭션이 빈번한 주간에 이 옵션을 사용하는 것은 금물이다.
...
27. DIRECT PATH INSERT가 빠른이유

a. UNDO 데이터를 남기지 않는다.
b. 테이블에 LOCK을 설정하지 않는다.
=> DIRECT PATH INSERT 시, DIRECT PATH INSERT EXCLUSIVE MODE TM LOCK을 설정하므로 다른 트랜재션이 해당 테이블에 DML 하지 못하게 된다.
...
ON (X.상품번호 = Y.상품번호)
WHERE MATCHED THEN UPDATE SET x.품질유지일 = Y.심규_품절유지일
```
