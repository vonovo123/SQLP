...
파티션 Pruning은 정적 /동적 파션 Pruning으로 구분된다.
...
동적 파티션 pruning 즉, 파티션 키 컬럼을 '바인드변수'로 조회하면 상수 조건일때와 달리 쿼리 최적화 시점에 액세스할 파티션을 미리 결정할 수 없게 된다.

실행 시점이 돼야 사용자가 입력한 값에 따라 액세스할 파티션이 결정되므로, 실행계획의 Pstart와 Pstop 컬럼에는 'Key'라고만 표시된다.

NL 조인시에도 Inner 테이블이 '조인컬럼' 기준으로 파티셔닝돼 있다면 동적 Pruning으로 작동한다.
...
바인드 변수를 사용해 동적 Prunning을 태스트한 것으로 시작과 종료 파티션 번호 대신 KEY라고 표시된다.
...
NL 조인에서도 Outer 테이블에서 읽히는 값에 따라 Inner Tavle 인 t 테이블에 동적 Prunning이 일어난다.
...
t 테이블은 no 컬럼 기준 Range Partitioning, Key 컬럼기준으로 해시파티셔닝돼있다.
...
바인드 변수를 사용할 때는 파티션 목록을 확정할 수 없어 Key라고 표시되지만 서브 파티션에서는 액세스할 주 파티션별로 16개씩 읽는다.
...
주 파티션과 서브파티션에 Pruning이 작동하면 하나의 서브파티션에서 49개의 블록만 읽는다.
...
이처럼 피타신 키 컬럼을 함부로 가공해서는 안된다. 묵시적 형 변환이 일어나는 경우도 마찬가지이다.
...
바인드 변수를 사용하면 최적화 시점에 엑세스할 파티션을 확정할 수 없어 '동적 파티션 Pruning'이 발생한다.

같은 이유로 쿼리 최적화에 '테이블 레벨 통계'가 사용된다.

반면, 정적 파티션 Pruning이 발생하는 경우는 '파티션 레벨 통계'가 사용된다.
...
위 과정을 수행한 후 B의 각 원소에 해시함수를 적용한 결과에 해당하는 Array의 비트 중 하나라도 0이면 그 원소는 집합 A에는 없는 값이다.
...
모두 1로 설정돼있음에도 실제로 확인해야 하는 이유는 잘못된 양수가 가능하기 때문이다. 즉, 집합 A에 없는 값인데 모두 1로 설정될 수 있다.

블룸 필터의 역할은 교집합에 해당하는 원소를 찾는 데 있지 않고 '교집합이 아닌 것이 확실한 원소'를 찾는 것이다.
