...
오라클은 병렬 방식으로 Full SCAN 할 때는 버퍼 캐시를 거치지 않고 곧바로 PGA 영역으로 읽어들이는 Direct Path Read 방식을 사용한다.
...
위 실행계획에서 병렬 서버는 T 테이블을 읽고서 갱신할 레코드 주소를 QC 전달하고 QC는 update를 수행한다.
...
T 테이블을 읽은 병렬 서버가 update까지 직접 수행하며, 갱신한 건수만 QC에 전달한다.

오라클 9iR1까지는 한 세그먼트를 두 개 이상 프로세스가 동시에 갱신할 수 없었다.

그래서 파티션되지 않은 테이블이라면 병렬로 갱신할 수 없었고, 파티션 테이블일때는 병렬도를 파티션 개수 이하로만 지정할 수 있었다.

병렬 DML이 파티션 기반 granule이었기 때문이다.

오라클 9iR2 부터는 병렬 DML이 블록기반 Granule로 바뀌었다.
...
데이터량에 비해 병렬도를 너무 크게 지정하면 노는 프로세스가 생긴다.
...
인덱스를 드라이빙 한 병렬 NL 조인에는 파티션 기반 Granule이 사용되므로 병렬도가 파티션 개수를 초과할 수 없다.

여기서는 세 개 파티션만 액세스하므로 병렬도를 3보다 크게줄 수 없다.
...
QC의 SQL 트레이스에 스칼라 서브쿼리 수행통계가 나타나지 않았으므로 병렬 서브 프로세스들이 스칼라 서브쿼리를 수행했음을 짐작할 수 있다.
...
각 병렬서버가 각각 세션 레벨 변수를 읽고 쓰면 테이블에 입력된 값의 중복이 발생한다.
