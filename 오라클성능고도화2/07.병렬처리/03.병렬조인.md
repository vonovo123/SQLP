...
### 1-1. 둘 다 같은 기준으로 파티셔닝 된 경우 - Full Partition Wise 조인
...
파티션 기준 컬럼으로 나눈 Partition-Pair가 10개라면, 두 개의 서버 프로세스가 각각 5개씩 순차적으로 처리한다.
...
양쪽 모두 파티셔닝해야 하므로 기본적으로 양쪽 테이블 모두에 대한 전체범위처리가 필요하다
...
위 쿼리 loc = 'CHICAGO' 조건을 만족하는 deptno만 조인에 성공하므로 이 조건을 만족하는 deptno만 해시 함수를 적용해 해당 비트들을 1로 설정한다.
...
두 테이블 중 작은 쪽을 반대편 서버 집합의 모든 프로세스에서 broadcast하고 나서 조인을 수행하는 방식이다.

- 1단계 : 첫 번째 서버 집합에 속한 프로세스들이 각자 읽은 테이블의 레코드를 두 번째 서버 집합에 속한 모든 병렬 프로세스에게 전송한다.

- 2단계 : 두 번째 서버 집합에 속한 프로세스들이 각자 맡은 범위의 emp 테이블을 읽으면서 조인을 수행한다.
...
테이블이 중대형 이상일 때는 과도한 프로세스간 통신으로인해 성능이 느려질 수 있다.
