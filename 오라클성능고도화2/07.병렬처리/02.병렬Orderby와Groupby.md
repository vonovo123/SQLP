...
hash group by가 실행됐고 위 쿼리에 order by를 추가하면 sort group by로 바뀐다

병렬 order by와 병렬 group by(+ order by)의 실행계획을 비교해보면 sort order by가 sort group by 로 바뀌는 것 밖에 없다.
...
group by의 결과를 QC에 전송할 때도 sort order by는 값 순서대로 진행하지만, hash group by는 먼저 처리가 끝난 순서대로 진행한다.
...
원래 group by를 담당하던 서버집합은 받은 데이터가 최종 집계된 값이 아니므로 한 번 더 group by를 진행한다.
