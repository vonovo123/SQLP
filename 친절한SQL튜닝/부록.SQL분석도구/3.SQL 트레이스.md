...
SQL을 튜닝할때 가장 많이 사용하는 도구는 SQL TRACE다. 앞서 살펴 본 사전 실행계획과 AUTOTRACE 결과만으로 문제점을 찾을 수 없을 때, SQL 트레이스를 이용하면 문제점을 쉽게 찾아낼 수 있다.
