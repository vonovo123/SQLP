...
일반적인 블록 I/O는 DB 버퍼캐시를 경유한다.

즉, 읽고자 하는 블록을 먼저 버퍼캐시에서 찾아보고, 찾지 못할 때만 디스크에서 읽는다.
...
select /*+full(o) parallel(0 4)*/ from 외부가입고객;
...
병렬 DML도 DIRECT PATH WRITE 방식을 사용하므로 데이터 입력/수정/삭제할 때 EXCLUSIVE 모드 TM LOCK이 걸린다는 사실을 기억하자.
